...
enum Commands {
    ...
    /// Useful for monitoring, dashboards, or running as a spawn-only hub.
    Listen(ListenCommand),
    /// Internal: wraps a CLI in a PTY with interactive passthrough.
    /// Used by the SDK and dashboard — not for direct user invocation.
...
struct ListenCommand {
    ...
    /// Port for the HTTP API that the dashboard proxies to (default: 3889)
    #[arg(long, default_value = "3889")]
    port: u16,

    /// Start the web dashboard (relay-dashboard-server) on port 3888
    #[arg(long)]
    dashboard: bool,

    /// Dashboard port (only used with --dashboard, default: 3888)
...
async fn main() -> Result<()> {
    ...
    let cli = Cli::parse();
    match cli.command {
        Commands::Init(cmd) => run_init(cmd).await,
        Commands::Pty(cmd) => run_pty_worker(cmd).await,
        Commands::Headless(cmd) => run_headless_worker(cmd).await,
        Commands::Listen(cmd) => run_listen(cmd).await,
        Commands::Wrap { cli, args } => {
            run_wrap(cli, args).await
        }
    }
}
...
async fn run_init(cmd: InitCommand) -> Result<()> {
    ...
    let auth = AuthClient::new(http_base.clone(), CredentialStore::new(paths.creds.clone()));
    let session = auth
        .startup_session(Some(&cmd.name))
        .await
    ...
    let (ws_inbound_tx, mut ws_inbound_rx) = mpsc::channel(512);
    ...
    let ws = RelaycastWsClient::new(
        ...
        session.token,
        session.credentials,
        channels_from_csv(&cmd.channels),
    );
    tokio::spawn(async move {
        ws.run(
            ws_inbound_tx,
    ...
    if let Err(error) = ensure_reaycast_mcp_config(
        ...
        tracing::warn!(
            ...
            error = %error,
            "failed to ensure .mcp.json relaycast server config"
        );
    }

    // Build a set of "self" agent names so we can filter echoes from both
    // the broker's own WS identity and the MCP agent identity used by
    // the hosted CLI (Claude Code, Codex, etc.).
    let self_names: HashSet<String> = {
        let mut names = HashSet::new();
        names.insert(cmd.name.clone());
        // The MCP server may register with the same name or a suffixed variant.
        // Read the configured RELAY_AGENT_NAME from .mcp.json to cover both.
            ...
            if let Ok(parsed) = serde_json::from_str::<Value>(&mcp_json) {
                    ...
                    .and_then(Value::as_str)
                {
                    if !mcp_name.is_empty() {
                        names.insert(mcp_name.to_string());
                    }
                }
            }
        }
        tracing::debug!(self_names = ?names, "echo filter identities");
        names
    };

    let self_agent_ids: HashSet<String> = {
        let mut ids = HashSet::new();
        ids.insert(self_agent_id.clone());
        ids
    };
                ...
                if let Some(ws_msg) = ws_msg {
                    if let Some(mapped) = map_ws_event(&ws_msg) {
                        ...
                        if self_names.contains(&mapped.from)
                            || mapped.sender_agent_id.as_ref().map_or(false, |id| self_agent_ids.contains(id))
                        {
    ...
    let _ = ws_control_tx.send(WsControl::Shutdown).await;
...
/// The HTTP API (default port 3889) is compatible with the relay-dashboard proxy mode.
/// Usage: `agent-relay listen --agent-name hub --channels general,ops --port 3889`
async fn run_listen(cmd: ListenCommand) -> Result<()> {
    ...
    let auth = AuthClient::new(http_base.clone(), CredentialStore::new(paths.creds.clone()));
    let session = auth
        .startup_session(Some(&requested_name))
        .await
        .context("failed to initialize relaycast session")?;
    let relay_workspace_key = session.credentials.api_key.clone();
    ...
    let self_names: HashSet<String> = {
        let mut names = HashSet::new();
        names.insert(agent_name.clone());
        names.insert(requested_name.clone());
        names
    };
    let self_agent_ids: HashSet<String> = {
        let mut ids = HashSet::new();
    ...
    // --- Optional dashboard ---
    let mut dashboard_child: Option<std::process::Child> = None;
    if cmd.dashboard {
        match find_dashboard_binary() {
            Some(bin) => {
                let relay_url = format!("http://127.0.0.1:{api_port}");
                match std::process::Command::new(&bin)
                    .args([
                        "--port",
                        &cmd.dashboard_port.to_string(),
                        "--relay-url",
                        &relay_url,
                    ])
                    ...
                    .spawn()
                {
                    Ok(child) => {
                        eprintln!(
                            "[agent-relay] dashboard starting on http://localhost:{}",
                            cmd.dashboard_port
                        );
                        dashboard_child = Some(child);
                    }
                    Err(e) => {
                        eprintln!("[agent-relay] failed to start dashboard: {e}");
                    }
                }
            }
            None => {
                eprintln!(
                    "[agent-relay] relay-dashboard-server not found — install it or run separately"
                );
                eprintln!("[agent-relay]   npx @agent-relay/dashboard-server --relay-url http://127.0.0.1:{api_port}");
            }
        }
    }
    ...
    while running {
        tokio::select! {
            ...
            // HTTP API requests from dashboard
            Some(req) = api_rx.recv() => {
                match req {
                    ListenApiRequest::Spawn { name, cli, args, reply } => {
                        let env_vars = [
                            ("RELAY_AGENT_NAME", name.as_str()),
                            ("RELAY_API_KEY", child_api_key.as_str()),
                            ("RELAY_BASE_URL", child_base_url.as_str()),
                            ("RELAY_CHANNELS", channels.as_str()),
                        ...
                        if let Some(cmd_event) = map_ws_broker_command(&ws_msg) {
                            ...
                            match cmd_event.payload {
                                BrokerCommandPayload::Spawn(ref params) => {
                                    ...
                                    let env_vars = [
                                        ("RELAY_AGENT_NAME", params.name.as_str()),
                                        ("RELAY_API_KEY", child_api_key.as_str()),
                                        ("RELAY_BASE_URL", child_base_url.as_str()),
                                        ("RELAY_CHANNELS", channels.as_str()),
                                        ("RELAY_STRICT_AGENT_NAME", "1"),
    ...
    if let Some(mut child) = dashboard_child {
        eprintln!("[agent-relay] stopping dashboard");
        let _ = child.kill();
        let _ = child.wait();
    }
...
async fn run_wrap(cli_name: String, cli_args: Vec<String>) -> Result<()> {
    ...
    // --- Auth & Relaycast connection (same as run_init) ---
    let runtime_cwd = std::env::current_dir()?;
    let paths = ensure_runtime_paths(&runtime_cwd)?;
    ...
    let session = auth
        .startup_session_with_options(Some(&requested_name), strict_name)
        .await
        .context("failed to initialize relaycast session")?;
    let relay_workspace_key = session.credentials.api_key.clone();

    // Use the actual registered name from Relaycast (may have a suffix if there was a conflict)
    let agent_name = session
        .credentials
        .agent_name
        .clone()
    ...
    // Build self-names set for echo filtering (covers both WS identity and MCP identity)
    let self_names: HashSet<String> = {
        let mut names = HashSet::new();
        names.insert(agent_name.clone());
        names.insert(requested_name.clone());
        ...
        tracing::debug!(self_names = ?names, "wrap-mode echo filter identities");
        names
    };

    // Collect all agent IDs in this workspace session for self-echo filtering.
    // The WS subscription delivers all workspace events, so any agent in our
    // workspace that shares the same credentials is "us".
    let self_agent_ids: HashSet<String> = {
        let mut ids = HashSet::new();
        ids.insert(self_agent_id.clone());
        ids
    ...
    // Auto-response state (same as run_pty_worker)
    ...
    let mut last_injection_time: Option<Instant> = None;
    ...
    let mut auto_suggestion_visible = false;
        ...
        tokio::select! {
            ...
            chunk = pty_rx.recv() => {
                match chunk {
                    Some(chunk) => {
                        ...
                        // Reset auto-enter on meaningful output
                        ...
                        if !is_echo && clean_text.len() > 10 && auto_enter_retry_count > 0 {
                            auto_enter_retry_count = 0;
                        }

                        // Extract message IDs from MCP tool responses to prevent self-echo.
                        // MCP responses contain JSON with "id": "<snowflake>" — pre-seed dedup
                        // so the WS echo is silently dropped.
                        {
                        ...
                        if !mcp_approved {
                            ...
                            let clean = strip_ansi(&mcp_detection_buffer);
                            let has_header = clean.contains("MCP Server Approval Required")
                                || clean.contains("MCP server approval");
                            let has_approve = clean.contains("[a] Approve all servers")
                                || clean.contains("Approve all")
                                || clean.contains("[a]");
                            if has_header && has_approve {
                                mcp_approved = true;
                        ...
                        // Bypass permissions
                        {
                            let in_cooldown = last_bypass_perms_send
                                ...
                                .unwrap_or(false);
                            if !in_cooldown && bypass_perms_send_count < BYPASS_PERMS_MAX_SENDS {
            ...
            _ = pending_injection_interval.tick() => {
                let should_block = pending_wrap_injections
                    .front()
                    .map(|pending| {
                        auto_suggestion_visible && pending.queued_at.elapsed() < AUTO_SUGGESTION_BLOCK_TIMEOUT
                    })
                    .unwrap_or(false);
                if should_block {
                    continue;
                ...
                if let Some(pending) = pending_wrap_injections.pop_front() {
                    if auto_suggestion_visible {
                        tracing::warn!(
                            event_id = %pending.event_id,
                            "auto-suggestion visible; sending Escape to dismiss before injection"
                        );
                        // Dismiss the auto-suggestion with Escape so it doesn't get accepted
                    ...
                    // Write message first, wait 50ms, then send Enter
                    let _ = pty.write_all(injection.as_bytes());
                    tokio::time::sleep(Duration::from_millis(50)).await;
                    let _ = pty.write_all(b"\r");
                    last_injection_time = Some(Instant::now());
                    auto_enter_retry_count = 0;
                }
            }
            ...
            _ = auto_enter_interval.tick() => {
                if let Some(injection_time) = last_injection_time {
                    let since_injection = injection_time.elapsed();
                    let since_output = last_output_time.elapsed();
                    let cooldown_ok = last_auto_enter_time
                        .map(|t| t.elapsed() >= AUTO_ENTER_COOLDOWN)
                        .unwrap_or(true);
                    let in_editor = is_in_editor_mode(&editor_mode_buffer);

                    if since_injection > AUTO_ENTER_TIMEOUT
                        && since_output > AUTO_ENTER_TIMEOUT
                        && cooldown_ok
                        && !in_editor
                        && !auto_suggestion_visible
                        && auto_enter_retry_count < MAX_AUTO_ENTER_RETRIES
                    {
                        let _ = pty.write_all(b"\r");
...
async fn run_pty_worker(cmd: PtyCommand) -> Result<()> {
    ...
    // --- Auto-response state (ported from relay-pty) ---
    let mut mcp_approved = false;
    let mut mcp_detection_buffer = String::new();
    let mut bypass_perms_buffer = String::new();
    let mut last_bypass_perms_send: Option<Instant> = None;
    ...
    let mut codex_model_prompt_handled = false;
    ...
    // Auto-send "2" to select "Allow for this session".
    ...
    // --- Auto-enter state for stuck agents ---
    let mut last_injection_time: Option<Instant> = None;
    let mut last_auto_enter_time: Option<Instant> = None;
    let mut auto_enter_retry_count: u32 = 0;
    const AUTO_ENTER_TIMEOUT: Duration = Duration::from_secs(10);
        ...
        tokio::select! {
            ...
            pty_output = pty_rx.recv() => {
                match pty_output {
                    Some(chunk) => {
                        ...
                        // Track output time for idle/auto-enter detection
                        last_output_time = Instant::now();

                        // Update editor mode buffer (last 2000 chars)
                        editor_mode_buffer.push_str(&text);
                        if editor_mode_buffer.len() > 2000 {
                            let start = floor_char_boundary(&editor_mode_buffer, editor_mode_buffer.len() - 1500);
                            editor_mode_buffer = editor_mode_buffer[start..].to_string();
                        ...
                        // Reset auto-enter on meaningful (non-echo) output
                        let clean_text = strip_ansi(&text);
                        let is_echo = clean_text.lines().all(|line| {
                            let trimmed = line.trim();
                            trimmed.is_empty() || trimmed.starts_with("Relay message from ")
                        ...
                        // --- MCP Server Approval auto-response ---
                        if !mcp_approved {
                            mcp_detection_buffer.push_str(&text);
                            if mcp_detection_buffer.len() > 2500 {
                                let start = floor_char_boundary(&mcp_detection_buffer, mcp_detection_buffer.len() - 2000);
                        ...
                        // --- Bypass permissions auto-response ---
                        {
                            let in_cooldown = last_bypass_perms_send
                                .map(|t| t.elapsed() < BYPASS_PERMS_COOLDOWN)
                                .unwrap_or(false);
                        ...
                        // --- Codex model upgrade prompt auto-dismiss ---
                        if !codex_model_prompt_handled {
                            ...
                            if has_upgrade_ref && has_model_options {
                                ...
                                codex_model_prompt_handled = true;
                                tokio::time::sleep(Duration::from_millis(100)).await;
                        ...
                        // --- Gemini "Action Required" auto-approval ---
                        {
            ...
            _ = pending_injection_interval.tick() => {
                ...
                if let Some(pending) = pending_worker_injections.pop_front() {
                    ...
                    // Write message first, wait 50ms, then send Enter.
                    // Proven pattern from relay-pty: CLIs need time to
                    // process pasted input before the submit keystroke.
                    let _ = pty.write_all(injection.as_bytes());
                    tokio::time::sleep(Duration::from_millis(50)).await;
                    let _ = pty.write_all(b"\r");
                    last_injection_time = Some(Instant::now());
...
/// Find the relay-dashboard-server binary.
/// Checks explicit install locations first, then falls back to PATH lookup.
fn find_dashboard_binary() -> Option<PathBuf> {
    let home = dirs::home_dir()?;
    let candidates = [
        home.join(".local/bin/relay-dashboard-server"),
        home.join(".agent-relay/bin/relay-dashboard-server"),
        PathBuf::from("/usr/local/bin/relay-dashboard-server"),
    ];
...
async fn terminate_child(child: &mut Child, timeout_duration: Duration) -> Result<()> {
    ...
    if tokio::time::timeout(timeout_duration, child.wait())
        .await
        .is_err()
    {
        #[cfg(unix)]
