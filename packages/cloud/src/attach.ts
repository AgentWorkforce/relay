...
export interface FleetNodeAttachOptions {
  /** Defaults to the only agent on the node; ambiguous nodes require an explicit agent. */
  agent?: string;
  node?: string;
  nodeId?: string;
  mode: AttachMode;
...
/** Internal compatibility transport for broker-based CLI attach clients. */
export interface FleetNodeAttachTransport {
  brokerUrl: string;
  apiKey: string;
  requestTimeoutMs: number;
}

export interface FleetNodeAttachProxy extends FleetNodeAttachTransport {
  /** Private UNIX byte-stream socket (named pipe on Windows). */
  socketPath: string;
  /** Inferred status: 0 on terminal.closed or detach, 1 on transport failure.
   * Relaycast does not currently transmit the remote process exit code. */
...
export async function startFleetNodeAttachProxy(
  ...
  const options = { ...input, node: input.node ?? input.nodeId ?? '', agent: input.agent ?? '' };
  if (input.node && input.nodeId && input.node !== input.nodeId) {
    throw new FleetNodeAttachError('node and nodeId must identify the same node.', 'invalid_node');
  }
  ...
  if (!options.agent) {
    const response = await fetchFn(`${baseUrl}/v1/nodes/${nodePath}/agents`, {
      headers: { Authorization: `Bearer ${workspaceKey}` },
      signal: AbortSignal.timeout(SESSION_REQUEST_TIMEOUT_MS),
    });
    const payload = (await response.json()) as { data?: Array<{ agentName?: string }> };
    if (!response.ok || !Array.isArray(payload.data)) {
      throw new FleetNodeAttachError('Could not discover the fleet node agent.', 'agent_discovery_failed');
    }
    const names = [
      ...new Set(
        payload.data
    ...
    if (names.length !== 1) {
      throw new FleetNodeAttachError(
        'Specify agent explicitly unless the node has exactly one agent.',
        'ambiguous_agent'
      );
    }
    options.agent = names[0];
  }
  ...
  try {
    socketPath =
      process.platform === 'win32'
        ? `\\\\.\\pipe\\agent-relay-attach-${randomBytes(16).toString('hex')}`
        : join((socketDirectory = await mkdtemp(join(tmpdir(), 'relay-attach-'))), 'stdio.sock');
  ...
  const endTerminal = (error: FleetNodeAttachError, eventCloseReason = error.message) => {
    ...
    terminalEnded = true;
    finish(error.code === 'terminal_closed' ? 0 : 1);
    rawSocket?.end();
    if (reconnectTimer) {
  ...
  const connect = (url: string, readiness: TerminalReadiness) => {
    ...
    socket.on('message', (data) => {
      ...
      if (frame.type === 'terminal.ready') {
        ...
        if (rawReady && rawSocket && !rawSocket.destroyed) {
          if (rawSocket.writableLength + Buffer.byteLength(frame.chunk) > MAX_BUFFERED_BYTES) {
            endTerminal(
              new FleetNodeAttachError(
                'terminal output exceeded the bounded socket buffer',
                'output_backpressure'
              )
            );
            return;
...
export type { AttachMode } from './attach-mode.js';
export { collectWithRetry } from './collect-with-retry.js';
