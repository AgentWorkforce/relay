...
const WORKFLOW_CONVENTIONS = [
  ...
  '- When you receive `Relay message from <sender> ...`, reply using `mcp__relaycast__send_dm(to: "<sender>", text: "...")`.',
  '- Send `ACK: ...` when you receive a task.',
  '- Send `DONE: ...` when the task is complete.',
  '- Do not reply only in terminal text; send the response via mcp__relaycast__send_dm.',
  '- Use mcp__relaycast__check_inbox() and mcp__relaycast__list_agents() when context is missing.',
].join('\n');

function hasWorkflowConventions(task: string): boolean {
  const lower = task.toLowerCase();
  return (
    lower.includes('mcp__relaycast__send_dm(') ||
