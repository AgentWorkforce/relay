...
if (!personaId) {
  ...
  if (found.length > 0) {
    ...
    console.error(
      'No personas found. Place JSON files under ./agentworkforce/personas ' + 'or set AGENT_WORKFORCE_HOME.'
    );
  }
  process.exit(1);
}
...
relay.addListener('agentSpawned', (agent) => console.log(`spawned ${agent.name} (${agent.runtime})`));
