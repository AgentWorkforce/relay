...
try {
  await build({
    ...
    nodePaths: [join(monorepoRoot, 'node_modules'), join(sdkRoot, 'node_modules')],
...
try {
  ...
  console.error('This is the shape of Cloudflare Workers error 10021 — some module in the');
  console.error("static graph of @agent-relay/sdk (via the 'workerd' export condition)");
  console.error('executes Node-only code at module load. Fix by making that code lazy,');
  console.error('or by narrowing the workerd entry (packages/sdk/src/workers.ts) so it');
...
try {
  ...
  if (body?.client !== 'function') {
    throw new Error(`consumer /ping expected AgentRelayClient to be a function, got ${JSON.stringify(body)}`);
  }
} catch (err) {
