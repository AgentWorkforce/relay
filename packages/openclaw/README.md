...
**Treat `message.post` as the real health check.** `status` and `agent.list` prove the workspace key and MCP registration are present, but they do **not** prove that the per-agent write token is usable.
