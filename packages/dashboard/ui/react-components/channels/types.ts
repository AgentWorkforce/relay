...
export interface ChannelMessageListProps {
  ...
  /** Current user name */
  currentUser: string;
  /** Channel members - used to detect @mentions of non-members */
...
export interface MessageInputProps {
  ...
  disabled?: boolean;
  /** Callback to send message (with optional attachment IDs) */
...
// =============================================================================
// Mention Invite Types
// =============================================================================

/**
 * Result of checking whether a mentioned user is a member of a channel.
 */
export interface MentionMembershipCheck {
  ...
  channelId: string;
  /** Channel visibility (invites only work for public channels) */
  channelVisibility: ChannelVisibility;
}

/**
 * API: Check membership for mentioned users in a channel.
 * POST /api/channels/:channelId/check-mentions
 */
export interface CheckMentionMembershipRequest {
  /** Usernames to check membership for */
  usernames: string[];
}

export interface CheckMentionMembershipResponse {
  /** Membership status for each mentioned user */
  results: MentionMembershipCheck[];
  /** Users who are not members and can be invited */
  nonMembers: string[];
}

/**
 * A channel invite sent to a non-member user via @mention.
 */
export interface MentionInvite {
  /** Unique invite ID */
  id: string;
  /** Channel the user is invited to */
  channelId: string;
  /** Channel name for display */
  channelName: string;
  /** Who sent the mention that triggered the invite */
  invitedBy: string;
  ...
  /** The message content containing the mention */
  messagePreview?: string;
}

/**
 * API: Send a channel invite via @mention.
 * POST /api/channels/:channelId/mention-invite
 */
export interface SendMentionInviteRequest {
  /** Username to invite */
  invitee: string;
  /** The message content that contained the mention */
  messageContent?: string;
}

export interface SendMentionInviteResponse {
  /** Whether the invite was sent successfully */
  success: boolean;
  /** The created invite (if successful) */
  invite?: MentionInvite;
  /** Error message (if failed) */
  error?: string;
  /** Whether the user was already a member */
  alreadyMember?: boolean;
  /** Whether the channel is private (invites not allowed) */
  channelIsPrivate?: boolean;
}

/**
 * API: Get pending invites for the current user.
 * GET /api/channels/invites
 */
export interface GetMentionInvitesResponse {
  /** Pending invites for the current user */
  invites: MentionInvite[];
}

/**
 * API: Respond to a mention invite.
 * POST /api/channels/invites/:inviteId/respond
 */
...
export interface RespondToInviteResponse {
  success: boolean;
  /** If accepted, the channel the user joined */
  channelId?: string;
  error?: string;
}
