...
export interface ChannelViewV1Props {
  ...
  onLoadMore?: () => void;
  /** Callback to send a message (with optional attachment IDs) */
  onSendMessage: (content: string, opts?: { threadId?: string; attachmentIds?: string[] }) => void;
...
export function ChannelViewV1({
  ...
  const handleSend = useCallback((content: string, attachmentIds?: string[]) => {
    onSendMessage(content, attachmentIds ? { attachmentIds } : undefined);
  }, [onSendMessage]);
