...
import type { AgentRelay, AgentRelayActions } from '@agent-relay/sdk';
...
export interface ACPBridgeConfig {
  ...
  agentName: string;
  /** Agent Relay API key. Defaults to AGENT_RELAY_API_KEY or RELAY_API_KEY. */
  apiKey?: string;
  /** Agent Relay API base URL. Defaults to AGENT_RELAY_BASE_URL or RELAY_BASE_URL. */
  baseUrl?: string;
  /** Channel used for untargeted ACP prompts. */
  channel?: string;
  /** Existing Relay facade for embedded/programmatic usage. */
  relay?: AgentRelay;
  /** Action registry used for optional driver actions such as agent.create. */
  actions?: AgentRelayActions;
  /** @deprecated ACP bridge now uses the public SDK messaging API instead of broker sockets. */
  socketPath?: string;
