...
import {
  ...
  type RelayAgent,
  type RelayMessage as SdkRelayMessage,
...
export class RelayACPAgent implements acp.Agent {
  ...
  private relayAgent: AgentRelayAgent | null = null;
  private connection: acp.AgentSideConnection | null = null;
  private relayEventUnsubscribers: Array<() => void> = [];
  ...
  async start(): Promise<void> {
    if (await this.connectRelay()) {
      this.debug('Connected to Agent Relay via SDK messaging');
    } else {
      this.debug('Failed to connect to Agent Relay via SDK messaging');
    }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      this.debug('Error during relay disconnect:', err);
    }
  ...
  private createRelay(): AgentRelay {
    ...
    return new AgentRelay({
      apiKey: this.config.apiKey ?? process.env.AGENT_RELAY_API_KEY ?? process.env.RELAY_API_KEY,
      baseUrl: this.config.baseUrl ?? process.env.AGENT_RELAY_BASE_URL ?? process.env.RELAY_BASE_URL,
      actions: this.config.actions,
    });
  }

  /**
   * Connect this ACP bridge as an Agent Relay participant.
   */
  private async connectRelay(): Promise<boolean> {
    try {
      const relay = this.relay ?? this.createRelay();
      this.relay = relay;

      const registration = await relay.agents.register({
        name: this.config.agentName,
        type: 'human',
        metadata: { adapter: 'acp-bridge' },
      });
      const agent = relay.as(registration);
      this.relayAgent = agent;

      await agent.channels.join(this.channel).catch((err) => {
        this.debug(`Unable to join channel ${this.channel}:`, err);
      });

      this.relayEventUnsubscribers = [
        agent.events.on('any', (event) => {
          if (
            event.type === 'messageCreated' ||
            event.type === 'threadReply' ||
            event.type === 'dmReceived' ||
            event.type === 'groupDmReceived'
          ) {
            this.handleSdkMessage(event.message);
          }
        }),
      ];
      agent.events.connect();
      agent.events.subscribe([this.channel]);
      return true;
    } catch (err) {
      this.debug('Agent Relay connection failed:', err);
      await this.disconnectRelay();
      return false;
    }
  }

  private async disconnectRelay(): Promise<void> {
    for (const unsubscribe of this.relayEventUnsubscribers.splice(0)) {
      unsubscribe();
    }
    await this.relayAgent?.events.disconnect().catch((err) => {
      this.debug('Agent Relay event disconnect failed:', err);
    });
    this.relayAgent = null;
    if (!this.config.relay) {
      this.relay = null;
    }
  }
  ...
  private async reconnectToRelay(): Promise<boolean> {
    ...
    this.debug(connected ? 'Reconnected to Agent Relay' : 'Failed to reconnect to Agent Relay');
    return connected;
  }
  ...
  private async ensureRelayReady(): Promise<boolean> {
    ...
    try {
      if (!this.relayAgent) return this.reconnectToRelay();
      await this.relayAgent.agents.presence();
      return true;
    } catch (err) {
    ...
    if (!(await this.ensureRelayReady())) {
      await this.connection.sessionUpdate({
        ...
        update: {
          ...
          content: {
            type: 'text',
            text: 'Agent Relay is not connected. Please check the bridge credentials and network connection.',
          },
        },
      });
      return {
    ...
    if (hasTargets) {
      ...
      for (const target of targets) {
        try {
          await agent.messages.direct({
            to: target,
            text: cleanMessage,
            idempotencyKey: `acp:${session.id}:${target}:${Date.now()}`,
          });
          sentCount += 1;
        } catch (err) {
      ...
      try {
        await agent.messages.send({
          ...
          text: userMessage,
          idempotencyKey: `acp:${session.id}:${Date.now()}`,
        });
        sentCount = 1;
      } catch (err) {
  ...
  private handleSdkMessage(message: SdkRelayMessage): void {
    const from = message.from.name ?? message.from.id ?? 'unknown';
    if (from === this.config.agentName) return;

    this.handleRelayMessage({
      id: message.id,
      from,
      body: message.text,
      thread: message.threadId ?? message.parentId,
  ...
  private async handleSpawnCommand(args: string[], sessionId: string): Promise<boolean> {
    ...
    if (!(await this.ensureRelayReady())) {
      await this.sendTextUpdate(sessionId, 'Agent Relay is not connected (cannot request agent.create).');
      return true;
    }
    ...
    try {
      const result = await this.invokeRelayAction('agent.create', {
        name,
        cli,
        task,
      ...
      if (!result.ok) {
        await this.sendTextUpdate(sessionId, `Spawn action failed for ${name}: ${result.error?.message}`);
        return true;
      }
      await this.sendTextUpdate(sessionId, `Requested agent.create for ${name}.`);
    } catch (err) {
  ...
  private async handleReleaseCommand(args: string[], sessionId: string): Promise<boolean> {
    ...
    if (!(await this.ensureRelayReady())) {
      await this.sendTextUpdate(sessionId, 'Agent Relay is not connected (cannot request agent.release).');
      return true;
    }
    ...
    try {
      const result = await this.invokeRelayAction('agent.release', { name });
      if (!result.ok) {
        await this.sendTextUpdate(sessionId, `Release action failed for ${name}: ${result.error?.message}`);
        return true;
      }
      await this.sendTextUpdate(sessionId, `Requested agent.release for ${name}.`);
    } catch (err) {
  ...
  private async handleListAgentsCommand(sessionId: string): Promise<boolean> {
    if (!(await this.ensureRelayReady())) {
      await this.sendTextUpdate(sessionId, 'Agent Relay is not connected (cannot list agents).');
      return true;
    }

    try {
      const agents = await this.relayAgent!.agents.list({ status: 'all' });
      if (!agents.length) {
        await this.sendTextUpdate(sessionId, 'No agents are currently connected.');
      } else {
        const lines = agents.map((agent: RelayAgent) => `- ${agent.name} (${agent.status})`);
  ...
  private async handleStatusCommand(sessionId: string): Promise<boolean> {
    ...
    if (isConnected) {
      try {
        const presence = await this.relayAgent!.agents.presence();
        lines.push(`Present agents: ${presence.length}`);
        lines.push(`Channel: ${this.channel}`);
      } catch (err) {
  ...
  private async invokeRelayAction(name: string, input: unknown) {
    if (!this.relay) {
      throw new Error('Agent Relay is not initialized.');
    }

    return this.relay.actions.invoke({
      name,
      input,
      caller: { name: this.config.agentName, type: 'human' },
    });
  }
  ...
  private getHelpText(): string {
    return [
      ...
      '',
      'Commands:',
      '- agent-relay spawn <name> <cli> "task"  (requires agent.create action)',
      '- agent-relay release <name>             (requires agent.release action)',
      '- agent-relay agents',
      '- agent-relay status',
      '- agent-relay help',
      '',
