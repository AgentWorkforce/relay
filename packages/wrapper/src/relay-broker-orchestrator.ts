/**
 * RelayBrokerOrchestrator - Orchestrates the agent-relay Rust binary
 *
 * This wrapper spawns the agent-relay binary and communicates via Unix socket.
 ...
 *
 * Architecture:
 * 1. Spawn agent-relay --name {agentName} -- {command} as child process
 * 2. Connect to socket for injection:
 *    - With WORKSPACE_ID: /tmp/relay/{workspaceId}/sockets/{agentName}.sock
 *    - Without: /tmp/agent-relay-{agentName}.sock (legacy)
 * 3. Parse stdout for relay commands (agent-relay echoes all output)
...
// ============================================================================
// Types for agent-relay socket protocol
// ============================================================================
...
/**
 * Request types sent to agent-relay socket
 */
...
type RelayBrokerRequest = InjectRequest | StatusRequest | ShutdownRequest | SendEnterRequest;

/**
 * Response types received from agent-relay socket
 */
...
/**
 * Configuration for RelayBrokerOrchestrator
 */
export interface RelayBrokerOrchestratorConfig extends BaseWrapperConfig {
  /** Path to agent-relay binary (default: searches PATH and ./relay-broker/target/release) */
  agentRelayPath?: string;
...
/**
 * Events emitted by RelayBrokerOrchestrator
 */
export interface RelayBrokerOrchestratorEvents {
...
/**
 * Orchestrator for agent-relay Rust binary
 *
 * Extends BaseWrapper to provide the same interface as PtyWrapper
 * but uses the agent-relay binary for improved injection reliability.
 */
export class RelayBrokerOrchestrator extends BaseWrapper {
  protected override config: RelayBrokerOrchestratorConfig;

  // Process management
  private agentRelayProcess?: ChildProcess;
  ...
  private static gatherSigkillDiagnostics(agentName: string, pid?: number): string {
    ...
    try {
      ...
      // Process count (try to get agent-relay count)
      try {
        const psOutput = execSync('ps aux | grep -c agent-relay || echo 0', { encoding: 'utf-8', timeout: 1000 }).trim();
        lines.push(`agent-relay processes: ${psOutput}`);
      } catch {
  ...
  constructor(config: RelayBrokerOrchestratorConfig) {
    super(config);
    this.config = config;
    ...
    if (workspaceId) {
      // Workspace mode: agent-relay watches the actual workspace path
      ...
      if (paths.socketPath.length > MAX_SOCKET_PATH_LENGTH) {
        ...
        console.warn(
          `[relay-broker-orchestrator:${config.name}] Socket path too long (${paths.socketPath.length} chars); using hashed workspace id ${hashedWorkspaceId}`
        );
        paths = hashedPaths;
      }
      ...
      // agent-relay watches the actual workspace path
  ...
  private log(message: string): void {
    if (this.config.debug) {
      const logLine = `${new Date().toISOString()} [relay-broker-orchestrator:${this.config.name}] ${message}\n`;
      try {
  ...
  private logError(message: string): void {
    const logLine = `${new Date().toISOString()} [relay-broker-orchestrator:${this.config.name}] ERROR: ${message}\n`;
    try {
      const logDir = dirname(this._logPath);
      if (!existsSync(logDir)) {
        mkdirSync(logDir, { recursive: true });
  ...
  /**
   * Start the agent-relay process and connect to socket
   */
  override async start(): Promise<void> {
    ...
    try {
      // Ensure the actual outbox directory exists (where agent-relay watches)
      ...
      if (this._workspaceId) {
        ...
        // This is the PRIMARY symlink - agents write to canonical path, agent-relay watches workspace path
    ...
    const binaryPath = this.findAgentRelayBinary();
    if (!binaryPath) {
      throw new Error('agent-relay binary not found. Build with: cd relay-broker && cargo build --release');
    }

    try {
      accessSync(binaryPath, fsConstants.X_OK);
    } catch (err: any) {
      throw new Error(`agent-relay binary not executable at ${binaryPath}: ${err?.message ?? 'permission denied'}. Build with: cd relay-broker && cargo build --release, or ensure the binary has execute permissions.`);
    }

    this.log(` Using binary: ${binaryPath}`);

    // Spawn agent-relay process FIRST (before connecting to daemon)
    // This ensures the CLI is actually running before we register with the daemon
    await this.spawnAgentRelay(binaryPath);
  ...
  /**
   * Stop the agent-relay process gracefully
   */
  override async stop(): Promise<void> {
    ...
    if (this.agentRelayProcess && !this.agentRelayProcess.killed) {
      this.agentRelayProcess.kill('SIGTERM');

      // Force kill after timeout
      await Promise.race([
        new Promise<void>((resolve) => {
          this.agentRelayProcess?.on('exit', () => resolve());
        }),
        sleep(5000).then(() => {
          if (this.agentRelayProcess && !this.agentRelayProcess.killed) {
            this.agentRelayProcess.kill('SIGKILL');
          }
        }),
      ]);
    }
   ...
   */
  private findAgentRelayBinary(): string | null {
    // Check config path first
    if (this.config.agentRelayPath && existsSync(this.config.agentRelayPath)) {
      return this.config.agentRelayPath;
    }
  ...
  /**
   * Spawn the agent-relay process
   */
  private async spawnAgentRelay(binaryPath: string): Promise<void> {
    ...
    // Handle stderr (agent-relay logs and JSON output) - always needed
    ...
    proc.on('exit', (code, signal) => {
      ...
      if (signal === 'SIGKILL' || exitCode === 137) {
        const diagnostics = RelayBrokerOrchestrator.gatherSigkillDiagnostics(this.config.name, proc.pid);
        this.logError(` SIGKILL DETECTED - gathering diagnostics:\n  ${diagnostics}`);
        console.error(`[relay-broker-orchestrator] SIGKILL for ${this.config.name}:\n  ${diagnostics}`);
      }
    ...
    if (proc.exitCode !== null) {
      ...
      throw new Error(`agent-relay exited immediately with code ${proc.exitCode}${stderrInfo}`);
    }
  ...
  /**
   * Handle output from agent-relay stdout (headless mode only)
  ...
  /**
   * Handle stderr from agent-relay (logs and JSON parsed commands)
   */
  private handleStderr(data: string): void {
    ...
    // agent-relay outputs JSON parsed commands to stderr with --json-output
      ...
      if (line.startsWith('{')) {
        ...
        try {
          ...
          if (parsed.type === 'relay_command' && parsed.kind) {
            ...
            // Handle continuity commands from agent-relay file-based protocol
          ...
          if (this.config.debug) {
            console.error(`[agent-relay:${this.config.name}] ${line}`);
          }
        }
      } else {
  ...
  /**
   * Handle a parsed command from Rust agent-relay
  ...
  /**
   * Handle continuity command from Rust agent-relay
   *
  ...
  private handleSpawnCommand(name: string, cli: string, task: string, cwd?: string): void {
    ...
    // Try daemon socket first (most reliable - daemon has agent-relay binary),
  ...
  private async executeSpawnWithFallbacks(name: string, cli: string, task: string, cwd?: string): Promise<void> {
    // 1. Try daemon socket spawn first (daemon always has access to agent-relay)
  ...
  /**
   * Check if the agent-relay process is still alive
   */
  private isProcessAlive(): boolean {
    if (!this.agentRelayProcess || this.agentRelayProcess.exitCode !== null) {
      return false;
    }
    try {
      // Signal 0 checks if process exists without killing it
      process.kill(this.agentRelayProcess.pid!, 0);
      return true;
    } catch {
      return false;
    }
  ...
  private async connectToSocket(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      // Check if agent-relay process died before attempting connection
      if (!this.isProcessAlive()) {
        ...
        if (exitInfo) {
          ...
          const diagnostics = exitInfo.signal === 'SIGKILL' || exitInfo.code === 137
            ? `\n  Diagnostics (SIGKILL often indicates OOM or resource limits):\n  ${RelayBrokerOrchestrator.gatherSigkillDiagnostics(this.config.name, this.agentRelayProcess?.pid)}`
            : '';

          throw new Error(`agent-relay process died early (${exitReason}).${stderrHint}${diagnostics}`);
        }
        throw new Error('agent-relay process died before socket could be created');
      }
    ...
    if (!this.isProcessAlive() && this.earlyExitInfo) {
      ...
      throw new Error(`agent-relay process died during socket connection (${exitReason}).${stderrHint}${diagnostics}`);
    }
   ...
   */
  private sendSocketRequest(request: RelayBrokerRequest): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.socket || !this.socketConnected) {
  ...
  private handleSocketResponse(line: string): void {
    try {
      const response = JSON.parse(line) as RelayBrokerResponse;
  ...
  private async handleInjectResult(response: InjectResultResponse): Promise<void> {
    ...
    if (response.status === 'delivered') {
      ...
      // Trust Rust's delivery confirmation - agent-relay writes directly to PTY which is very reliable.
      //
...
📖 See **AGENTS.md** in the project root for full protocol documentation.`;
   ...
   * This requires:
   * 1. agent-relay process spawned
   * 2. Socket connected to agent-relay
   * 3. running flag set
   *
  ...
  /**
   * Get the agent-relay process PID
   */
  get pid(): number | undefined {
    return this.agentRelayProcess?.pid;
  }

  /**
   * Get the log file path (not used by agent-relay, returns undefined)
   */
  ...
  async write(data: string | Buffer): Promise<void> {
    if (!this.agentRelayProcess || !this.agentRelayProcess.stdin) {
      throw new Error('Process not running');
    }
    const buffer = typeof data === 'string' ? Buffer.from(data) : data;
    this.agentRelayProcess.stdin.write(buffer);
  }
