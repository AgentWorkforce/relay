...
const WORKFLOW_ROOT = 'packages/broker-sdk/src/workflows';
...
TIER 4 — Storage Backends:
  ...
  All three: exported from packages/broker-sdk/src/workflows/db/ submodule
...
5. Establish a non-negotiable constraint: ALL existing tests in
   packages/broker-sdk/src/__tests__/ must remain passing after each phase
...
Also update packages/broker-sdk/src/workflows/builder.ts:
...
8. Run: npx tsc --noEmit from packages/broker-sdk and confirm zero errors
...
8. Existing tests: run npx jest packages/broker-sdk and report pass/fail counts
...
WORKFLOW REGISTRY:
Create packages/broker-sdk/src/workflows/registry.ts:
...
YOUR TASK: Create packages/broker-sdk/src/workflows/auto-builder.ts
...
Create a new directory: packages/broker-sdk/src/workflows/db/

Create these files:

1. packages/broker-sdk/src/workflows/db/postgres.ts
...
2. packages/broker-sdk/src/workflows/db/sqlite.ts
...
3. packages/broker-sdk/src/workflows/db/redis.ts
...
4. packages/broker-sdk/src/workflows/db/index.ts
   export { PostgresWorkflowDb } from './postgres.js';
   export { SqliteWorkflowDb } from './sqlite.js';
   export { RedisWorkflowDb } from './redis.js';

5. Update packages/broker-sdk/src/workflows/index.ts to export from db/:
...
REVIEW packages/broker-sdk/src/workflows/registry.ts,
       packages/broker-sdk/src/workflows/auto-builder.ts,
       packages/broker-sdk/src/workflows/db/:
...
YOUR TASK: Create packages/broker-sdk/src/workflows/server.ts
...
Also create packages/broker-sdk/src/workflows/serve.ts — the CLI entry point:
...
2. Create packages/broker-sdk/src/workflows/tracing.ts:
...
First, locate the CLI code (likely in packages/broker-sdk/src/workflows/cli.ts):
  cat packages/broker-sdk/src/workflows/cli.ts
...
Also update the README at packages/broker-sdk/src/workflows/README.md to document
...
6. Run npx jest packages/broker-sdk and report pass/fail counts
7. Run npx tsc --noEmit and report results
...
1. TYPE CHECK:
   cd packages/broker-sdk && npx tsc --noEmit
   Report: zero errors or list all errors

2. EXISTING TESTS:
   npx jest packages/broker-sdk/src/__tests__/
...
5. EXPORTS CHECK:
   Verify that the following are exported from packages/broker-sdk/src/workflows/index.ts:
...
End with: IMPLEMENTATION_COMPLETE
  ...
  .run({
    ...
    vars: {
      ...
      // workflowRoot: 'packages/broker-sdk/src/workflows',
    },
  });
