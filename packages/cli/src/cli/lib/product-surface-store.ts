 ...
 */
const LOADER_FILENAME = 'load-surface.mjs';

const LOADER_SOURCE = `// Generated by agent-relay. Do not edit.
//
// Resolves the specifier against the node_modules beside this file, by hand,
// and imports the result as an absolute URL.
//
// Neither a bare dynamic import nor import.meta.resolve works here. Both
// succeed under Node and both fail inside the compiled binary: Bun does not
// resolve a scoped package with an exports subpath, such as
// @relayfile/sdk/relay-cli, from a module it loaded by absolute path. It
// reports "Cannot find package" though the package sits in the sibling tree.
// An unscoped package without exports resolves fine, which is what makes the
// failure easy to miss.
//
// Importing a fully resolved absolute URL avoids the resolver entirely.
import { readFileSync } from 'node:fs';
import path from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';

function selectCondition(entry) {
  if (typeof entry === 'string') return entry;
  if (!entry || typeof entry !== 'object') return undefined;
  // Same precedence Node applies for an ESM import of a dual package.
  for (const condition of ['import', 'module', 'node', 'default']) {
    const value = entry[condition];
    if (typeof value === 'string') return value;
    if (value && typeof value === 'object') {
      const nested = selectCondition(value);
...
export function importSurface(specifier) {
  const here = path.dirname(fileURLToPath(import.meta.url));
  const segments = specifier.split('/');
  const name = specifier.startsWith('@') ? segments.slice(0, 2).join('/') : segments[0];
  const subpath = '.' + specifier.slice(name.length);

  const packageDir = path.join(here, 'node_modules', ...name.split('/'));
  const manifest = JSON.parse(readFileSync(path.join(packageDir, 'package.json'), 'utf8'));

  const exported = manifest.exports;
  let relative;
  if (exported === undefined) {
    // No exports map: the legacy fields are the only entry points.
    relative = subpath === '.' ? (manifest.module ?? manifest.main ?? 'index.js') : specifier.slice(name.length + 1);
  } else if (typeof exported === 'string') {
    relative = subpath === '.' ? exported : undefined;
  } else {
    relative = selectCondition(exported[subpath]);
  }

  if (!relative) {
    throw new Error(
      'agent-relay: ' + name + ' does not export ' + subpath + ' (resolved from ' + packageDir + ')'
    );
  }
  return import(pathToFileURL(path.join(packageDir, relative)).href);
}
`;
...
export async function provisionSurfacePackage(
  ...
  if (fs.existsSync(path.join(target, LOADER_FILENAME))) return target;
...
async function provisionOnce(
  ...
  try {
    ...
    fs.writeFileSync(path.join(staging, LOADER_FILENAME), LOADER_SOURCE);
    ...
    try {
      ...
      if (!fs.existsSync(path.join(target, LOADER_FILENAME))) throw error;
    }
...
/**
 * Import `specifier` out of a provisioned tree.
 *
 * Goes through the generated loader rather than `createRequire(...).resolve`:
 * the product entry points are ESM behind `exports` subpaths, which CommonJS
 * resolution reports as not exported.
 *
 * @param installRoot - Directory returned by {@link provisionSurfacePackage}.
 * @param specifier - Bare specifier, e.g. `@relayfile/sdk/relay-cli`.
 */
export async function importFromSurfaceStore(installRoot: string, specifier: string): Promise<unknown> {
  const loaderUrl = pathToFileURL(path.join(installRoot, LOADER_FILENAME)).href;
  const loader = (await import(loaderUrl)) as {
    importSurface?: (specifier: string) => Promise<unknown>;
  };
  if (typeof loader.importSurface !== 'function') {
    throw new SurfaceProvisionError(
      `The provisioned package tree at ${installRoot} is damaged (${LOADER_FILENAME} exports no loader).\n` +
        `Delete that directory and retry; it will be reinstalled.`
    );
  }
  return loader.importSurface(specifier);
}
