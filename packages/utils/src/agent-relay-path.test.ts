/**
 * Comprehensive tests for agent-relay binary path resolution.
 *
...
describe('findAgentRelayBinary - search path verification', () => {
  beforeEach(() => {
    clearBinaryCache();
  ...
  describe('npx installation (scoped @agent-relay/* package)', () => {
    ...
    // ~/.npm/_npx/{hash}/node_modules/agent-relay/bin/agent-relay-darwin-arm64
    ...
    it('should include correct npx cache path for scoped package', () => {
      ...
      findAgentRelayBinary(callerDirname);
    ...
    it('should check platform-specific binary BEFORE generic binary', () => {
      ...
      const platformIdx = paths.findIndex(
        (p) => p.startsWith(npxBasePath) && p.includes('agent-relay-')
      );
      const genericIdx = paths.findIndex((p) => p === `${npxBasePath}/agent-relay`);
  ...
  describe('npx installation (direct agent-relay package)', () => {
    // When CLI code itself calls findAgentRelayBinary:
  ...
  describe('Development (monorepo)', () => {
    ...
    it('should include Rust target/release path', () => {
      ...
      expect(paths.some((p) => p.includes('relay-broker/target/release/agent-relay'))).toBe(true);
    });
    ...
    it('should include Rust target/debug path', () => {
      ...
      expect(paths.some((p) => p.includes('relay-broker/target/debug/agent-relay'))).toBe(true);
    });
  });

  describe('Docker container', () => {
    it('should include /app/bin/agent-relay path', () => {
      const callerDirname = '/app/node_modules/@agent-relay/bridge/dist';

      findAgentRelayBinary(callerDirname);
      const paths = getLastSearchPaths();

      expect(paths).toContain('/app/bin/agent-relay');
    });
  });

  describe('System-wide install', () => {
    it('should include /usr/local/bin/agent-relay path', () => {
      const callerDirname = '/some/path';

      findAgentRelayBinary(callerDirname);
      const paths = getLastSearchPaths();

      expect(paths).toContain('/usr/local/bin/agent-relay');
    });
  });
  ...
  describe('Platform-specific binary naming', () => {
    it('should include platform-specific binary name in search paths', () => {
      ...
      if (platform === 'darwin' && arch === 'arm64') {
        expectedBinaryName = 'agent-relay-darwin-arm64';
      } else if (platform === 'darwin' && arch === 'x64') {
        expectedBinaryName = 'agent-relay-darwin-x64';
      } else if (platform === 'linux' && arch === 'arm64') {
        expectedBinaryName = 'agent-relay-linux-arm64';
      } else if (platform === 'linux' && arch === 'x64') {
        expectedBinaryName = 'agent-relay-linux-x64';
      }
  ...
  describe('Search path order priority', () => {
    it('should have platform-specific binaries before generic binaries for same location', () => {
      ...
      for (const binDir of binDirs) {
        const platformIdx = paths.findIndex(
          (p) => p.startsWith(binDir) && p.includes('agent-relay-')
        );
        const genericIdx = paths.findIndex((p) => p === `${binDir}/agent-relay`);
  ...
  describe('Bash installer (curl | bash)', () => {
    // install.sh places binary at ~/.agent-relay/bin/agent-relay
    // and optionally at ~/.local/bin/agent-relay
    ...
    it('should respect AGENT_RELAY_INSTALL_DIR override', () => {
      ...
      findAgentRelayBinary('/some/path');
      const paths = getLastSearchPaths();
  ...
  describe('Environment variable override', () => {
    ...
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(nodePath.join(os.tmpdir(), 'agent-relay-env-test-'));
    });
    ...
    it('should use AGENT_RELAY_BINARY when file is executable and platform-compatible', () => {
      ...
      const binPath = nodePath.join(tmpDir, 'agent-relay');
      ...
      const result = findAgentRelayBinary('/any/path');
      expect(result).toBe(binPath);
    });

    it('should fall back to normal search when AGENT_RELAY_BINARY file does not exist', () => {
      process.env.AGENT_RELAY_BINARY = '/nonexistent/path/agent-relay';

      findAgentRelayBinary('/any/path');
      const paths = getLastSearchPaths();
...
describe('isPlatformCompatibleBinary - cross-platform binary validation', () => {
  ...
  beforeEach(() => {
    clearBinaryCache();
    tmpDir = fs.mkdtempSync(nodePath.join(os.tmpdir(), 'agent-relay-test-'));
  });
  ...
  it('should accept correct binary for current platform via AGENT_RELAY_BINARY', () => {
    ...
    const binPath = createFakeBinary('agent-relay', magic);
    process.env.AGENT_RELAY_BINARY = binPath;
  ...
  it('should reject wrong-platform binary via AGENT_RELAY_BINARY', () => {
    const platform = process.platform;
  ...
  if (process.platform === 'darwin') {
    it('should accept MH_MAGIC_64 (native byte order) on macOS', () => {
      const binPath = createFakeBinary('agent-relay', MACHO_MAGIC_64);
      process.env.AGENT_RELAY_BINARY = binPath;
    ...
    it('should accept fat/universal binaries on macOS', () => {
      const binPath = createFakeBinary('agent-relay', FAT_MAGIC);
      process.env.AGENT_RELAY_BINARY = binPath;
  ...
  if (process.platform === 'linux') {
    it('should reject Mach-O binaries on Linux', () => {
      const binPath = createFakeBinary('agent-relay', MACHO_CIGAM_64);
      process.env.AGENT_RELAY_BINARY = binPath;
