...
mod tests {
    ...
    #[test]
    fn node_heartbeat_is_a_minimal_liveness_frame() {
        // The heartbeat carries ONLY live load/liveness — the node roster
        // (name, node_id, capabilities, max_agents, version) is owned by
        // `node.register`, and `last_heartbeat_at` is stamped server-side by
        // relaycast on receipt. The engine's `node.heartbeat` schema is strict,
        // so any extra roster fields here would be rejected wholesale and drop
        // the live load update. This guards that wire contract.
