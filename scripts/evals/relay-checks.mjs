export function assertRelayExpected(testCase, actualInput) {
  ...
  for (const item of asArray(expected.reactionCount)) {
    ...
    const message = actual.messages.find((candidate) => candidate.id === messageId || candidate.messageId === messageId);
  ...
  for (const item of asArray(expected.channelMembers)) {
    ...
    const observed = actual.channels.find((candidate) => candidate.name === normalizeChannelName(channel))?.members ?? [];
...
function normalizeRelayActual(actualInput) {
  ...
  return {
    ...
    events: Array.isArray(observed.events) ? observed.events : Array.isArray(actual.events) ? actual.events : [],
...
function messageMatches(message, check) {
  if (check.channel !== undefined && message.channel?.name !== normalizeChannelName(check.channel)) return false;
...
function deepRead(value, key) {
  return String(key).split(".").reduce((current, part) => current?.[part], value);
}
