...
function requireOptionKeys(options, names, command) {
  const actual = Object.keys(options).sort();
  const expected = [...names].sort();
  if (actual.join('\0') !== expected.join('\0')) {
    throw new Error(
      `${command} arguments must exactly match ${expected.map((name) => `--${name}`).join(', ')}`
    );
  }
}

export function candidateIdentityFromOptions(options) {
  return {
    sourceSha: requiredString(options['source-sha'], '--source-sha', SHA40),
    packageVersion: requiredString(options['package-version'], '--package-version', VERSION),
  };
}
...
async function hydrate(attestationPath, tarballDirectory, outputRoot, expectedIdentity) {
  const sourceAttestation = path.resolve(attestationPath);
  const sourceBytes = await readRegularFileNoFollow(sourceAttestation, {
    label: 'portable candidate attestation',
    privateMode: true,
    currentUserOwned: true,
  }).then((result) => result.bytes);
  const { sourceSha, packageVersion } = expectedIdentity;
...
async function main() {
  ...
  if (command === 'hydrate') {
    requireOptionKeys(
      options,
      ['attestation', 'tarballs', 'output', 'source-sha', 'package-version'],
      command
    );
    await hydrate(
      requiredString(options.attestation, '--attestation'),
