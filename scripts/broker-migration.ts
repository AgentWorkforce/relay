      ...
      tasks: [
        {
          ...
          prompt: [
            ...
            priorHandoff,
            "",
            "Implement two features in the Rust broker. The workers have detailed instructions",
            "with exact file paths and code snippets. Your job is to coordinate and unblock them.",
            "",
            "Key files:",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/helpers.rs` — add ActivityDetector + ThrottleState structs",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs` — integrate activity + throttle",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/wrap.rs` — same integration",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs` — handle delivery_active events (~line 890)",
            "",
            "**Worker-W4-Activity** builds: ActivityDetector (per-CLI patterns), delivery_active events",
            "**Worker-W4-Throttle** builds: ThrottleState (ring buffer, backoff math), injection delay",
            "",
            "Both workers modify helpers.rs (new structs) + pty_worker.rs + wrap.rs.",
            "IMPORTANT: If both try to edit the same file, coordinate to avoid conflicts.",
            "Suggest Worker-W4-Activity focuses on helpers.rs ActivityDetector + pty_worker.rs/wrap.rs activity code,",
            "and Worker-W4-Throttle focuses on helpers.rs ThrottleState + pty_worker.rs/wrap.rs throttle code.",
            "",
            "Use `/Users/khaliqgant/.cargo/bin/cargo test` to verify (not just `cargo test`).",
            "When both workers are DONE, print HANDOFF to stdout.",
          ].join("\n"),
        },
        {
          role: "worker",
          ...
          prompt: [
            ...
            "",
            "Working directory: /Users/khaliqgant/Projects/agent-workforce/relay",
            "",
            "### What to build",
            "After a message is delivery_verified (echo detected in PTY output), continue monitoring",
            "PTY output for signs the agent is actively processing the message. Emit a `delivery_active`",
            "worker event when activity is detected.",
            "",
            "### Files to modify",
            "",
            "**1. `/Users/khaliqgant/Projects/agent-workforce/relay/src/helpers.rs`**",
            "Add an `ActivityDetector` struct with per-CLI-type activity patterns:",
            "```rust",
            ...
            "    pub fn detect_activity(&self, output: &str) -> bool { ... }",
            "}",
            "```",
            "Activity patterns by CLI type:",
            "- claude: `⠋`, `⠙`, `⠹` (thinking spinners), `Tool:`, `Read(`, `Write(`, `Edit(`",
            "- codex: `Thinking...`, `Running:`, `$ `, `function_call`",
            "- gemini: `Generating`, `Action:`, `Executing`",
            "- default: any non-empty output within the window",
            "",
            "**2. `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs`**",
            "After a `delivery_verified` event is emitted (around line 202), start an activity",
            "detection window. The PendingVerification struct is in helpers.rs. Add a new",
            "`PendingActivity` struct or extend PendingVerification. When PTY output matches",
            "an activity pattern within 5s of verification, emit a `delivery_active` worker event:",
            "```rust",
            "serde_json::json!({",
            "    \"type\": \"delivery_active\",",
            "    \"payload\": { \"delivery_id\": id, \"event_id\": eid, \"pattern\": matched }",
            "})",
            "```",
            "",
            "**3. `/Users/khaliqgant/Projects/agent-workforce/relay/src/wrap.rs`**",
            "Same change as pty_worker.rs — after delivery_verified (around line 675+),",
            "add activity detection window.",
            "",
            "**4. `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs`**",
            "Handle `delivery_active` worker events (around line 890, after delivery_verified handler):",
            "```rust",
            ...
            "",
            "### Testing",
            "Add unit tests in helpers.rs for ActivityDetector pattern matching.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo check` then `/Users/khaliqgant/.cargo/bin/cargo test`.",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          role: "worker",
          ...
          prompt: [
            ...
            "",
            "### What to build",
            "Per-worker injection rate control. Track delivery outcomes and adjust delay between",
            "injections. Healthy workers get fast delivery, struggling workers get backoff.",
            "",
            "### Files to modify",
            "",
            "**1. `/Users/khaliqgant/Projects/agent-workforce/relay/src/helpers.rs`**",
            "Add a ThrottleState struct:",
            "```rust",
            "#[derive(Debug, Clone)]",
            ...
            "    outcomes: VecDeque<DeliveryOutcome>, // ring buffer, max 10",
            "    current_delay_ms: u64,",
            "}",
            "impl ThrottleState {",
            "    pub fn new() -> Self { Self { outcomes: VecDeque::new(), current_delay_ms: 100 } }",
            "    pub fn record(&mut self, outcome: DeliveryOutcome) { ... }",
            "    pub fn delay(&self) -> Duration { ... }",
            "}",
            "```",
            "Throttle logic:",
            "- Healthy (0-1 failures in last 10): 100ms delay",
            "- Struggling (2-4 failures): 200ms → 500ms → 1s exponential",
            "- Failing (5+ failures): 2s → 5s cap",
            "- Recovery: after 3 consecutive successes, halve current delay (min 100ms)",
            "",
            "**2. `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs`**",
            "At the top of run_pty_worker() (around line 25), create a ThrottleState instance.",
            "Before each PTY injection (around line 175 where `pending_worker_injections.pop_front()`",
            "is called), apply `tokio::time::sleep(throttle.delay())` first.",
            "After delivery_verified, call `throttle.record(DeliveryOutcome::Success)`.",
            "After delivery_failed, call `throttle.record(DeliveryOutcome::Failed)`.",
            "",
            "**3. `/Users/khaliqgant/Projects/agent-workforce/relay/src/wrap.rs`**",
            "Same throttle integration as pty_worker.rs — create ThrottleState, apply delay",
            "before injection, record outcomes after verification.",
            "",
            "### Testing",
            "Add unit tests in helpers.rs for ThrottleState:",
            "- test_throttle_healthy: 10 successes → 100ms delay",
            "- test_throttle_backoff: 5 failures → delay increases",
            "- test_throttle_recovery: failures then 3 successes → delay halves",
            "Run `/Users/khaliqgant/.cargo/bin/cargo check` then `/Users/khaliqgant/.cargo/bin/cargo test`.",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "Working directory: /Users/khaliqgant/Projects/agent-workforce/relay",
            "",
            "Review the Rust implementation in these files:",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/helpers.rs` — ActivityDetector + ThrottleState",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs` — activity + throttle integration",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/wrap.rs` — same integration",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs` — delivery_active event handler",
            "",
            "Check:",
            "1. Activity patterns are realistic for each CLI (claude spinners, codex Thinking, etc.)",
            "2. ThrottleState math: backoff on failure, recovery on 3 consecutive successes",
            "3. Ring buffer stays bounded at 10 entries",
            "4. No regressions in existing delivery flow",
            "",
            "Run `/Users/khaliqgant/.cargo/bin/cargo test` — all tests must pass.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo clippy` — no new warnings.",
            "Print REVIEW:PASS or REVIEW:FAIL to stdout.",
          ].join("\n"),
        },
      ],
    },
      ...
      tasks: [
        ...
        },
        {
          ...
          prompt: [
            ...
            "",
            "### What to build",
            "Add granular delivery lifecycle events so SDK consumers can track message delivery progress.",
            "Event chain: delivery_queued → delivery_injected → delivery_verified → delivery_active",
            "Or on failure: delivery_queued → delivery_injected → delivery_failed (reason)",
            "",
            "### Files to modify",
            "",
            "**1. `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs`**",
            "Currently emits delivery_verified and delivery_failed. Add two new events:",
            "- `delivery_queued`: emit when a message is added to pending_worker_injections (around line 85)",
            "- `delivery_injected`: emit right after PTY write_all (around line 185), before verification starts",
            "Each event payload: `{ delivery_id, event_id, worker_name, timestamp }`",
            "Use `std::time::SystemTime::now().duration_since(UNIX_EPOCH)` for timestamp.",
            "",
            "**2. `/Users/khaliqgant/Projects/agent-workforce/relay/src/wrap.rs`**",
            "Same changes as pty_worker.rs — add delivery_queued and delivery_injected events.",
            "",
            "**3. `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs`**",
            "Handle the new worker events in the worker event loop (around line 880-930).",
            "Forward them to the SDK as broker events via send_event(), same pattern as delivery_verified:",
            "```rust",
            ...
            "```",
            "",
            "**4. `/Users/khaliqgant/Projects/agent-workforce/relay/packages/sdk-ts/src/protocol.ts`**",
            "Add `delivery_queued` and `delivery_injected` to the BrokerEvent kind union type.",
            "",
            "**5. `/Users/khaliqgant/Projects/agent-workforce/relay/packages/sdk-ts/src/relay.ts`**",
            "Add an `onDeliveryUpdate` hook (alongside onEvent) that fires for all delivery_* events.",
            "Wire it in the wireEvents() method.",
            "",
            "### Testing",
            "Add unit tests in pty_worker.rs or helpers.rs to verify events are emitted in order.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo check` then `/Users/khaliqgant/.cargo/bin/cargo test`.",
            "Run `cd /Users/khaliqgant/Projects/agent-workforce/relay/packages/sdk-ts && npm run build`.",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "Working directory: /Users/khaliqgant/Projects/agent-workforce/relay",
            "",
            "Review these files for delivery receipt implementation:",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs` — delivery_queued + delivery_injected events",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/wrap.rs` — same events",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs` — event forwarding to SDK",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/packages/sdk-ts/src/protocol.ts` — BrokerEvent types",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/packages/sdk-ts/src/relay.ts` — onDeliveryUpdate hook",
            "",
            "Verify event chain: delivery_queued → delivery_injected → delivery_verified → delivery_active.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo test` — all tests must pass.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo clippy` — no new warnings.",
            "Print REVIEW:PASS or REVIEW:FAIL to stdout.",
          ].join("\n"),
      ...
      tasks: [
        ...
        },
        {
          ...
          prompt: [
            ...
            "",
            "### What to build",
            "Enable the broker to reattach to orphaned agent processes after a crash/restart.",
            "",
            "### Files to modify",
            "",
            "**1. `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs`**",
            "The PersistedAgent struct (around line 135) already has `pid` and `started_at` fields.",
            "The `reap_dead_agents()` method (around line 2329) already removes dead agents.",
            "",
            "Add a `reattach_live_agents()` method to BrokerState that:",
            "- Iterates persisted agents with live PIDs (is_pid_alive() already exists)",
            "- For each live agent, reattach by opening /proc/{pid}/fd/0 (Linux) or using",
            "  the saved PTY path to reconnect to the agent's terminal",
            "- Re-register them in WorkerRegistry",
            "- Emit agent_reattached events to SDK",
            "",
            "Call reattach_live_agents() in run_init() after reap_dead_agents() (around line 688).",
            "",
            "**2. `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs`**",
            "Add support for spawning with `setsid` (process group leader) so agents survive",
            "broker crashes. Use `std::os::unix::process::CommandExt::pre_exec` with `libc::setsid()`.",
            "This goes in the Command builder around line 40-60.",
            "",
            "### Testing",
            "Add unit tests for reattach logic.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo check` then `/Users/khaliqgant/.cargo/bin/cargo test`.",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "",
            "### What to build",
            "Persist pending (undelivered) messages to disk so they survive broker restarts.",
            "",
            "### Files to modify",
            "",
            "**1. `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs`**",
            "The PendingDelivery struct (around line 294) holds undelivered messages.",
            "Add persistence:",
            "- `save_pending(path, deliveries)`: serialize pending_deliveries HashMap to JSON",
            "- `load_pending(path)`: deserialize on startup",
            "- Save after each delivery is queued (use .agent-relay/pending.json)",
            "- Load in run_init() after reattach, retry pending deliveries to reattached workers",
            "",
            "Use the same atomic write pattern as BrokerState::save() (tempfile + rename).",
            "",
            "### Testing",
            "Add unit test: serialize/deserialize round-trip for pending deliveries.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo check` then `/Users/khaliqgant/.cargo/bin/cargo test`.",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "",
            "Review these files:",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/main.rs` — reattach_live_agents(), pending delivery persistence",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/src/pty_worker.rs` — setsid process group",
            "",
            "Check:",
            "1. Reattach logic correctly identifies live orphaned processes",
            "2. setsid is applied to spawned agents so they survive broker crashes",
            "3. Pending deliveries are persisted atomically (tempfile + rename)",
            "4. Loaded pending deliveries are retried on restart",
            "",
            "Run `/Users/khaliqgant/.cargo/bin/cargo test` — all tests must pass.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo clippy` — no new warnings.",
            "Print REVIEW:PASS or REVIEW:FAIL to stdout.",
      ...
      tasks: [
        ...
        },
        {
          ...
          prompt: [
            ...
            "",
            "### What to build",
            "Create a benchmark suite comparing broker message delivery vs old Relaycast-only stack.",
            "",
            "### Files to create",
            "",
            "**1. `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/harness.ts`**",
            "Benchmark runner that:",
            "- Spawns broker + 2 test agents using AgentRelayClient from @agent-relay/broker-sdk",
            "- Measures operation timing with process.hrtime.bigint()",
            "- Supports --quick flag for CI (fewer iterations)",
            "- Outputs results as a comparison table",
            "",
            "**2. `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/latency.ts`**",
            "Measure send_message round-trip time (SDK → broker → PTY → verified).",
            "Run 100 iterations (10 in --quick mode), report p50/p95/p99.",
            "",
            "**3. `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/throughput.ts`**",
            "Send N messages in rapid succession, measure msgs/sec delivery rate.",
            "",
            "**4. `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/cold-start.ts`**",
            "Measure time from broker spawn to first successful message delivery.",
            "",
            "Import SDK from @agent-relay/broker-sdk (packages/sdk-ts).",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "### Files to create",
            "",
            "**1. `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/reliability.ts`**",
            "Send 1000 messages, count delivery_verified vs delivery_failed. Report success rate.",
            "",
            "**2. `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/overhead.ts`**",
            "Measure broker CPU/memory overhead: RSS before/after spawning agents.",
            "Use process.memoryUsage() and /proc/self/stat if available.",
            "",
            "**3. `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/scale-out.ts`**",
            "Spawn 2, 5, 10 agents and measure message delivery latency at each scale.",
            "Report how latency degrades with agent count.",
            "",
            "Import SDK from @agent-relay/broker-sdk (packages/sdk-ts).",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "Working directory: /Users/khaliqgant/Projects/agent-workforce/relay",
            "",
            "Review benchmark files in `/Users/khaliqgant/Projects/agent-workforce/relay/tests/benchmarks/`.",
            "Run `npx tsx tests/benchmarks/latency.ts --quick` to verify it produces output.",
            "Check measurement correctness (hrtime usage, proper warmup, statistical reporting).",
            "Run `/Users/khaliqgant/.cargo/bin/cargo test` — no regressions.",
            "Print REVIEW:PASS or REVIEW:FAIL to stdout.",
          ].join("\n"),
        },
      ],
      ...
      tasks: [
        ...
        },
        {
          ...
          prompt: [
            ...
            "Working directory: /Users/khaliqgant/Projects/agent-workforce/relay",
            "",
            "Port these old SDK tests to broker-sdk equivalents. The old tests are in",
            "`/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/sdk/`.",
            "The new tests go in `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/broker/`.",
            "",
            "Read each old test file first, then rewrite using:",
            "- `@agent-relay/broker-sdk` imports (AgentRelayClient, AgentRelay, etc.)",
            "- The broker harness from `tests/integration/broker/utils/broker-harness.ts`",
            "- node:test runner (import { test, describe } from 'node:test')",
            "",
            "Files to create:",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/broker/16-broadcast.ts`",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/broker/17-multi-worker.ts`",
            "- `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/broker/19-orch-to-worker.ts`",
            "",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "Working directory: /Users/khaliqgant/Projects/agent-workforce/relay",
            "",
            "Port and create test files:",
            "",
            "1. Read `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/sdk/15-continuity-handoff.js`",
            "   Rewrite as `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/broker/18-continuity-handoff.ts`",
            "   using @agent-relay/broker-sdk and node:test runner.",
            "",
            "2. Create `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/broker/20-stability-soak.ts`",
            "   A 5-minute soak test that sends messages at steady intervals, counts",
            "   delivery_verified vs delivery_failed, and reports success rate over time.",
            "   Use the broker harness from `tests/integration/broker/utils/broker-harness.ts`.",
            "",
            "When done, print DONE to stdout.",
          ].join("\n"),
        },
        {
          ...
          prompt: [
            ...
            "Working directory: /Users/khaliqgant/Projects/agent-workforce/relay",
            "",
            "Review test files in `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/broker/`:",
            "- 16-broadcast.ts, 17-multi-worker.ts, 18-continuity-handoff.ts,",
            "  19-orch-to-worker.ts, 20-stability-soak.ts",
            "",
            "Compare against old tests in `/Users/khaliqgant/Projects/agent-workforce/relay/tests/integration/sdk/`.",
            "Verify imports use @agent-relay/broker-sdk, not old SDK paths.",
            "Run `/Users/khaliqgant/.cargo/bin/cargo test` — no regressions.",
            "Print REVIEW:PASS or REVIEW:FAIL to stdout.",
          ].join("\n"),
        },
