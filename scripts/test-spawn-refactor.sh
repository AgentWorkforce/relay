...
print_checklist() {
  ...
  echo "  2. Start LOCAL daemon:    node packages/cli/dist/cli/index.js up --dashboard --port $DASHBOARD_PORT"
  echo "  3. Ensure ANTHROPIC_API_KEY is set"
  echo ""
  echo -e "${BOLD}Test 1: Spawn via Local CLI (daemon socket path)${NC}"
  echo "  node packages/cli/dist/cli/index.js spawn TestWorker claude 'Say hello then wait' --port $DASHBOARD_PORT"
  echo "  Expected: Agent spawns via daemon socket (check daemon log for 'SPAWN' envelope)"
  echo "  Verify:   node packages/cli/dist/cli/index.js agents --port $DASHBOARD_PORT"
  echo "  Cleanup:  node packages/cli/dist/cli/index.js release TestWorker --port $DASHBOARD_PORT"
  echo ""
  ...
  echo "  1. Spawn an agent: node packages/cli/dist/cli/index.js spawn FallbackTest claude 'wait'"
  echo "  2. Try spawning same name again: node packages/cli/dist/cli/index.js spawn FallbackTest claude 'wait'"
  echo "  Expected: Second spawn gets daemon rejection, does NOT fall through to HTTP"
  echo "  Cleanup:  node packages/cli/dist/cli/index.js release FallbackTest"
  echo ""
...
check "test -f packages/cli/dist/cli/index.js" "Relay CLI built"
