...
const EXPECTED_CANDIDATE_SHA = process.env.VERIFY_FLEET_EXPECTED_RELAY_SHA?.trim();
const EXPECTED_CANDIDATE_VERSION = process.env.VERIFY_FLEET_EXPECTED_RELAY_VERSION?.trim();
...
if (
  Boolean(CONFIGURED_CANDIDATE_CLI) &&
  (!/^[a-f0-9]{40}$/.test(EXPECTED_CANDIDATE_SHA ?? '') ||
    !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(EXPECTED_CANDIDATE_VERSION ?? ''))
) {
  throw new Error(
    'configured candidates require VERIFY_FLEET_EXPECTED_RELAY_SHA and VERIFY_FLEET_EXPECTED_RELAY_VERSION'
  );
}
...
async function main() {
  ...
  const wf = workflow('relay-fleet-daytona-comprehensive')
    .description(
      'Run the 120-operation Relay Fleet and node-agent catalog twice, each time on two fresh Daytona nodes with five critical targeted lifecycle trials, zero ambient identities, executable candidate attestation, exact cleanup, repeatability classification, and fresh Claude/Codex evidence signoff.'
    )
    .pattern('dag')
