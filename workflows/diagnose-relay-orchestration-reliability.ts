...
function finalSignoffTask(provider: 'claude' | 'codex'): string {
  ...
  return [
    ...
    'Check bug deduplication, owners, reproductions, open unknowns, all 95 Fleet operations, cleanup, disposable-workspace qualification, exact candidate snapshot identity, and promotion prohibition.',
  ...
  const wf = workflow('diagnose-relay-orchestration-reliability')
    ...
    .step('author-coverage-contract', {
      ...
      task: [
        ...
        `Read the required transition/fault/acceptance ids in ${GATE} and all 95 operations in tests/relayflows/cleanroom/fleet-daytona.matrix.json.`,
        'Use schemaVersion 1, kind relay-orchestration-coverage, and mode diagnosis.',
        'Create exactly 143 unique rows: 12 transitions, 23 fault cases, 13 acceptance gates, and 95 Fleet operations.',
