...
function reviewTask(reviewer: 'claude' | 'codex', final: boolean): string {
  ...
  return [
    ...
    'Check that every failed static gate and every one of the 153 diagnosis coverage rows is represented by a bidirectional bug/unknown mapping, and that snapshot/prerelease qualification cannot pass on a stale image.',
...
function finalSignoffTask(provider: 'claude' | 'codex'): string {
  ...
  return [
    ...
    'Check bug deduplication, owners, reproductions, open unknowns, all 105 Fleet operations, cleanup, disposable-workspace qualification, exact candidate snapshot identity, and promotion prohibition.',
  ...
  const wf = workflow('diagnose-relay-orchestration-reliability')
    ...
    .step('author-coverage-contract', {
      ...
      task: [
        ...
        `Read the required transition/fault/acceptance ids in ${GATE} and all 105 operations in tests/relayflows/cleanroom/fleet-daytona.matrix.json.`,
        'Use schemaVersion 1, kind relay-orchestration-coverage, and mode diagnosis.',
        'Create exactly 153 unique rows: 12 transitions, 23 fault cases, 13 acceptance gates, and 105 Fleet operations.',
