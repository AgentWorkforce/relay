...
| Interactive agents on a channel whose task strings don't tell them to talk to each other                                           | Pick a [Question / Answer (blocking ask)](#1-question--answer-blocking-ask), [Broadcast / Ack](#2-broadcast--ack), [Peer Review Handoff](#3-peer-review-handoff), [Standup / Status Probe](#4-standup--status-probe), or [Hand-Off with Context](#5-hand-off-with-context) recipe and bake it into the task prompt — otherwise you're paying for a chat substrate you're not using |
