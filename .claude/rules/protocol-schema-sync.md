---
paths:
  - "relay-broker/src/protocol.rs"
  - "relay-broker/src/parser.rs"
...
When modifying relay-broker protocol types, you MUST update ALL corresponding schemas:

| Source of Truth | Must Update |
|-----------------|-------------|
| `relay-broker/src/protocol.rs` | TypeScript schemas + JSON schemas |
| `relay-broker/src/parser.rs` (headers) | All file format schemas |

## Files That Must Stay Synchronized

### 1. Rust Protocol Types (Source of Truth)
- `relay-broker/src/protocol.rs` - `ParsedRelayCommand`, `InjectRequest`, `InjectResponse`, `SyncMeta`
- `relay-broker/src/parser.rs` - Header parsing (`TO:`, `KIND:`, `AWAIT:`, etc.)

### 2. TypeScript Schemas
- `packages/protocol/src/relay-broker-schemas.ts` - TypeScript interfaces matching Rust types
...
2. **Update TypeScript** (`relay-broker-schemas.ts`)
...
```typescript
// 3. relay-broker-schemas.ts - Add to interfaces
...
- [ ] TypeScript interfaces updated (`relay-broker-schemas.ts`)
