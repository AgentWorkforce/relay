...
BROKER_SRC_DIR="$REPO_ROOT/crates/broker/src"
...
#
# Bug (investigation finding): all 3 spawn paths in crates/broker/src/main.rs hardcoded
...
SPAWN_CHANNELS_FN=$(awk '/^fn default_spawn_channels/,/^\}/' "$BROKER_SRC_DIR/main.rs" | head -20)
if echo "$SPAWN_CHANNELS_FN" | grep -q '"engineering"'; then
    ...
    fail "Test 8a: default_spawn_channels() does NOT include 'engineering' — agents miss #engineering (bug: crates/broker/src/main.rs)"
fi

# Check 8b: all 3 spawn paths must use default_spawn_channels() (not hardcoded vec!["general"])
GENERAL_ONLY_LINES=$(grep -n 'channels: vec!\["general"' "$BROKER_SRC_DIR/main.rs" 2>/dev/null | grep -v '///' || true)
if [ -z "$GENERAL_ONLY_LINES" ]; then
    pass "Test 8b: no hardcoded channels:[\"general\"] spawn sites remain"
else
    fail "Test 8b: hardcoded channels:[\"general\"] spawn sites found — engineering never joined (bug: crates/broker/src/main.rs)"
fi

# Check 8c: all spawn paths call default_spawn_channels() — must appear at least 3 times
DEFAULT_SPAWN_CALLS=$(grep -c 'default_spawn_channels()' "$BROKER_SRC_DIR/main.rs" 2>/dev/null | tr -d ' ')
...
#   crates/broker/src/snippets.rs:557-581 only passed RELAY_API_KEY and RELAY_BASE_URL —
...
GEMINI_FN_BODY=$(awk '/^fn gemini_droid_mcp_add_args/,/^\}/' "$BROKER_SRC_DIR/snippets.rs" | head -50)

if echo "$GEMINI_FN_BODY" | grep -q 'RELAY_AGENT_TOKEN'; then
    pass "Test 9a: gemini_droid_mcp_add_args injects RELAY_AGENT_TOKEN"
else
    fail "Test 9a: gemini_droid_mcp_add_args does NOT inject RELAY_AGENT_TOKEN — gemini MCP unauthenticated (bug: crates/broker/src/snippets.rs)"
fi
...
#
# Bug (investigation finding): ensure_opencode_config() at crates/broker/src/snippets.rs:313
...
OPENCODE_FN_SIG=$(grep -A8 '^pub fn ensure_opencode_config' "$BROKER_SRC_DIR/snippets.rs")

if echo "$OPENCODE_FN_SIG" | grep -q 'agent_token'; then
    pass "Test 10a: ensure_opencode_config accepts relay_agent_token parameter"
else
    fail "Test 10a: ensure_opencode_config has NO agent_token parameter — cannot write RELAY_AGENT_TOKEN (bug: crates/broker/src/snippets.rs)"
fi

OPENCODE_FN_BODY=$(awk '/^pub fn ensure_opencode_config/,/^\}/' "$BROKER_SRC_DIR/snippets.rs" | head -80)
...
CALL_SITE=$(grep -A3 'ensure_opencode_config(cwd' "$BROKER_SRC_DIR/snippets.rs" 2>/dev/null || true)
...
CURSOR_CFG_FN=$(awk '/^pub fn ensure_cursor_mcp_config/,/^\}/' "$BROKER_SRC_DIR/snippets.rs" | head -60)
...
IS_CURSOR_DETECT=$(grep -n 'is_cursor' "$BROKER_SRC_DIR/snippets.rs" | head -5)
...
echo "Note: Source fixes applied in uncommitted changes to crates/broker/src/*.rs"
echo "═══════════════════════════════════════════════════════════"
