...
test("functionality: worker_stream — CLI output captured as events", { timeout: 90_000 }, async (t) => {
  ...
  try {
    ...
    assert.ok(
      totalBytes > 100,
      `total worker_stream output should be > 100 bytes, got ${totalBytes}`,
    );
...
test("functionality: priority — P1 and P4 messages both delivered", { timeout: 120_000 }, async (t) => {
  ...
  try {
    ...
    assert.ok(
      acks.length >= 2,
      `both priority messages should be delivered, got ${acks.length} acks`,
    );
...
test("functionality: bypass prompt — Claude starts with --dangerously-skip-permissions", { timeout: 90_000 }, async (t) => {
  if (skipIfMissing(t)) return;
  ...
  try {
    ...
    assert.ok(
      acks.length >= 1,
      "Claude should process messages after bypass prompt auto-handling",
    );

    // Check worker_stream output doesn't contain stuck permission prompts
    const streams = eventsForAgent(events, agentName, "worker_stream");
    const fullOutput = streams
      .map((e) => (e as BrokerEvent & { chunk: string }).chunk)
      .join("");
    ...
    for (const pattern of stuckPatterns) {
      ...
      assert.ok(
        !lastChunk.includes(pattern),
        `agent should not be stuck on "${pattern}" prompt`,
      );
    }
...
test("functionality: worker_stream during message processing — output captured", { timeout: 120_000 }, async (t) => {
  if (skipIfMissing(t)) return;
  const cli = skipUnlessAnyCli(t);
  if (!cli) return;
  ...
  try {
    ...
    assert.ok(
      totalBytes > 200,
      `should capture > 200 bytes of processing output, got ${totalBytes}`,
    );
...
test("functionality: channel isolation — messages route to correct agent", { timeout: 120_000 }, async (t) => {
  if (skipIfMissing(t)) return;
  const cli = skipUnlessAnyCli(t);
  if (!cli) return;
