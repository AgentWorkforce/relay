 ...
 */
function getRelayInboundFromAgent(
  events: BrokerEvent[],
  agentName: string
): BrokerEvent[] {
  ...
  async (t) => {
    ...
    try {
      ...
      assert.ok(
        ...
        "Claude: DM should hint to use mcp__relaycast__dm_send"
      );
      assert.ok(
...
test(
  "mcp-injection: claude — channel message injects post_message hint",
  { timeout: 90_000 },
  async (t) => {
    if (skipIfMissing(t)) return;
    if (skipIfNotRealCli(t)) return;
    ...
    try {
      ...
      assert.ok(
        ...
        "Claude: channel hint should mention the channel name"
      );
...
test(
  "mcp-injection: claude — agent uses MCP tools to respond (e2e)",
  { timeout: 180_000 },
  async (t) => {
    if (skipIfMissing(t)) return;
    if (skipIfNotRealCli(t)) return;
    ...
    try {
      await harness.spawnAgent(agentName, "claude", ["general"], {
        task: "You are a test agent. When you receive a message, respond using the mcp__relaycast__dm_send tool to reply directly to the sender. Keep responses to one sentence.",
      });
      await sleep(15_000);
...
test(
  "mcp-injection: gemini — pre-spawn mcp add enables MCP tools",
  { timeout: 180_000 },
  async (t) => {
    if (skipIfMissing(t)) return;
    if (skipIfNotRealCli(t)) return;
...
test(
  "mcp-injection: droid — pre-spawn mcp add enables MCP tools",
  { timeout: 180_000 },
  async (t) => {
    if (skipIfMissing(t)) return;
    if (skipIfNotRealCli(t)) return;
...
test(
  "mcp-injection: aider — no MCP injection, PTY delivery still works",
  { timeout: 90_000 },
  async (t) => {
    if (skipIfMissing(t)) return;
    if (skipIfNotRealCli(t)) return;
    ...
    try {
      ...
      console.log(
        `  Aider relay_inbound responses (expected 0): ${agentResponses.length}`
      );
...
test(
  "mcp-injection: goose — no MCP injection, PTY delivery still works",
  { timeout: 90_000 },
  async (t) => {
    if (skipIfMissing(t)) return;
    if (skipIfNotRealCli(t)) return;
    ...
    try {
      ...
      const agentResponses = getRelayInboundFromAgent(events, agentName);
      console.log(
        `  Goose relay_inbound responses (expected 0): ${agentResponses.length}`
      );
  ...
  async (t) => {
    ...
    try {
      ...
      await harness.spawnAgent(claudeName, "claude", ["general"], {
        task: `You are a test agent named ${claudeName}. When you receive a message, respond using mcp__relaycast__dm_send to reply to the sender. Keep responses to one sentence.`,
      });
      await harness.spawnAgent(codexName, "codex", ["general"], {
        task: `You are a test agent named ${codexName}. When you receive a message, respond using mcp__relaycast__dm_send to reply to the sender. Keep responses to one sentence.`,
      });
