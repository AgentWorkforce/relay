...
describe('complete Daytona Fleet board', () => {
  ...
  it('enumerates the complete Fleet and node-agent command/provider board', async () => {
    ...
    expect(matrix.operations).toHaveLength(121);
    ...
    expect(Object.keys(matrix.acceptance.operationProfiles)).toHaveLength(121);
  ...
  it('binds every operation record to an executable acceptance profile', async () => {
    ...
    expect(() => validateFleetAcceptance(missing)).toThrow(/exactly map all 121/);
  });
  ...
  it('validates the candidate inventory independently from the trusted CLI collector', async () => {
    ...
    // The trusted verifier can land before the candidate-only CLI additions it
    // describes. Exercise the collector against the trusted current CLI, then
    // validate the immutable candidate inventory fixture that the live board
    // compares to the hydrated candidate before any operation receives credit.
    expect(actual.commands.length).toBeGreaterThan(0);
    ...
    expect(expected.commands.find(({ path }: { path: string }) => path === 'fleet serve')).toMatchObject({
      hidden: true,
      leaf: true,
    });
    expect(
    ...
    expect(() => compareFleetCliInventory(expected, missingCommand)).toThrow('inventory changed');
  ...
  it('rejects duplicate operations and an incomplete provider board', async () => {
    ...
    expect(() => validateFleetMatrix(wrongCount)).toThrow(/exactly 121/);
    ...
    expect(() => validateFleetMatrix(incomplete)).toThrow(/must exactly map all 121 operations/);
  });
...
`
  ...
  it('binds valid reviews to the exact immutable evidence seal', () => {
    ...
    const review = {
      ...
      deterministicEvidence: ['121 exact operation records'],
  ...
  it('classifies mixed repeated outcomes as flaky and rejects sandbox reuse', async () => {
    ...
    expect(green.operationTotals).toEqual({
      matrixOperationCount: 121,
      independentCommandExecutionCount: 119,
