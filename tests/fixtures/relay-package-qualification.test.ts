...
describe('Relay package qualification producer', () => {
  ...
  it('binds the two exact package artifacts before the trusted consumer hydrates the candidate', async () => {
    ...
    expect(downloadAttestation).toBeGreaterThan(-1);
    expect(verifyBundle).toBeGreaterThan(downloadPayload);
    expect(verifyBundle).toBeGreaterThan(downloadAttestation);
    expect(hydrate).toBeGreaterThan(verifyBundle);
    expect(workflow).toContain('--source-sha "$RELAY_SHA"');
    expect(workflow).toContain('--package-version "$version"');
    expect(workflow).not.toMatch(/ref:\s*\$\{\{\s*(?:github\.event\.workflow_run\.)?head_sha\s*\}\}/);
