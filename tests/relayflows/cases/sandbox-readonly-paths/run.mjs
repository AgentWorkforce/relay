...
const CASE_ID = 'sandbox-readonly-paths';
...
test('observes exact Relayfile subtree forwarding', async () => {
  const observationPath = process.env.RELAY_READONLY_OBSERVATION_PATH;
  if (!observationPath) throw new Error('Missing RELAY_READONLY_OBSERVATION_PATH.');
  ...
  await ensureCloudFleetSandbox({
    ...
    mountRelayfile: true,
    readonlyPaths: ['/live-review/run-123/**', '/live-review/run-123/reviews/**'],
  });
  ...
  await writeFile(
    observationPath,
    JSON.stringify({ readonlyPaths: ensureBody.readonlyPaths ?? null }),
    'utf8'
  );
});
`;
...
const probeConfigSource = `export default {
  test: {
    environment: 'node',
    include: ['packages/cloud/src/.relayflow-sandbox-readonly-paths.test.ts'],
    setupFiles: [],
  },
};\n`;
...
try {
  ...
  run(
    ...
    { RELAY_READONLY_OBSERVATION_PATH: probeObservationPath }
  );
  ...
  const cliHasScopedMountFlag = cliHelp.includes('--sandbox-readonly-path <path...>');
  const requestedPaths = ['/live-review/run-123/**', '/live-review/run-123/reviews/**'];
  const baseObserved = observation.readonlyPaths === null && !cliHasScopedMountFlag;
  const headObserved =
    JSON.stringify(observation.readonlyPaths) === JSON.stringify(requestedPaths) && cliHasScopedMountFlag;
  ...
  if (baseObserved) {
    outcome = 'absent';
    signature = 'sandbox_readonly_paths_not_forwarded';
    details =
      'The base Cloud client dropped requested Relayfile subtrees and the fleet spawn CLI exposed no read-only-mount flag.';
  } else if (headObserved) {
    outcome = 'fixed';
    signature = 'sandbox_readonly_paths_forwarded';
    details =
      'The head Cloud client forwarded the exact requested Relayfile subtrees and the fleet spawn CLI exposed the read-only-mount flag.';
  } else {
    throw new Error(
      `Unexpected read-only Relayfile mount observation: ${JSON.stringify({
        ...observation,
