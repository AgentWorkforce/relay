...
The `flows/ci/pr-proof.flow.ts` Relayflow is deployed as a hosted listener
(`flows deploy … --on github:events=pull_request`). Cloud launches it with the
listener's event as run input; the flow reduces that envelope to a pull request
number and lets `prepare.mjs` resolve the authoritative pull request from the
API, so no listener-supplied SHA, title, or body reaches the proof contract. It
then fetches and validates the case manifest as data. PR code runs only in the
two Cloud agent sandboxes. Each prover uploads structured evidence through the
run-scoped Cloud storage API; the case runner never receives Cloud credentials.
A cryptographic nonce binds both handoffs to the current run. The base proof
must pass its deterministic evidence gate before the head sandbox is launched,
and the final gate rejects a stale nonce, identical sandbox IDs, or incorrect
commit provenance.
