...
try {
  ...
  run(
    'engine',
    process.execPath,
    [engineBin, '--port', String(port), '--db', path.join(directory, 'engine.sqlite'), '--env', 'test'],
  ...
  run('broker', binary, ['init', '--api-port', '0', '--api-bind', '127.0.0.1', '--state-dir', state], {
    ...
    env: {
      ...
      RELAYCAST_BASE_URL: origin,
      RELAY_API_KEY: key,
      RELAY_WORKSPACE_KEY: key,
      RELAY_NODE_ID: nodeId,
      RELAY_NODE_TOKEN: node.value.data.token,
  ...
  const spawned = await wait(async () => {
    const response = await api('/api/spawn', 'POST', {
  ...
  for (const [label, text] of Object.entries(logs))
    await writeFile(path.join(directory, `${label}-private.log`), text, { mode: 0o600 });
