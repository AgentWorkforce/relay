...
async function stopRelayPty(instance: RelayPtyInstance): Promise<void> {
  ...
  try {
    if (existsSync(instance.socketPath)) unlinkSync(instance.socketPath);
  } catch {}
}

// ── Broker Helpers ───────────────────────────────────────────────────────────

async function startBrokerWithAgent(name: string): Promise<{ client: AgentRelayClient; name: string }> {
...
async function testLatency(): Promise<void> {
  ...
  try {
    ...
    try {
      await client.release(brokerName);
    } catch {}
  ...
  const winner =
    brokerStats.p50 < relayStats.p50
...
async function testMultiAgent(): Promise<void> {
  ...
  try {
    ...
    for (const agent of brokerAgents) {
      try {
        await brokerClient.release(agent);
      } catch {}
    }
  ...
  try {
    ...
    const relayStats = computeStats(relaySamples);
    console.log(
      `  Relay-PTY:   total=${relayTotal.toFixed(0)}ms  avg_latency=${relayStats.mean.toFixed(2)}ms`
    );
...
function printFeatureComparison(): void {
  ...
  const relayFeatures = features.filter(
    ([_, __, r]) => !r.startsWith('N/A') && r !== 'No' && !r.startsWith('No ')
  ).length;
  ...
  for (const [_, broker, relay] of features) {
    if (
      (relay === 'N/A' ||
        relay.startsWith('No') ||
        relay.startsWith('App-level') ||
        relay.startsWith('Manual') ||
        relay.startsWith('Exit callback') ||
        relay.startsWith('Raw') ||
        relay.startsWith('1 ')) &&
      !broker.startsWith('N/A') &&
      !broker.startsWith('No')
    ) {
      brokerAdvantages++;
    }
  }
