...
/// hand the bytes off to a `std::sync::mpsc` channel; a dedicated
/// drainer thread takes the writer lock and pushes them down the PTY,
/// decoupling potential `write_all` blocking from the parser hot path.
#[derive(Clone)]
...
impl RelayEventListener {
    fn new(tx: std_mpsc::Sender<Vec<u8>>) -> Self {
        Self { tx }
    }
}
...
impl EventListener for RelayEventListener {
    fn send_event(&self, event: Event) {
        ...
        if let Event::PtyWrite(text) = event {
            // Best-effort send. If the drainer thread has exited (PTY
            // teardown), silently drop — telemetry/logging must never
            // break terminal emulation.
