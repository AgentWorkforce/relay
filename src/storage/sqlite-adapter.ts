...
export class SqliteStorageAdapter implements StorageAdapter {
  ...
  /**
   * Get all channel memberships for a specific agent/user.
   * Uses window function to get the most recent membership action per channel.
   * Only returns channels where the most recent action is 'join'.
   * @param memberName - The agent or user name
   * @returns Array of channel names (with # prefix)
   */
  async getChannelMembershipsForAgent(memberName: string): Promise<string[]> {
    ...
    // Query messages with _channelMembership metadata for this agent
    // Use window function to get most recent action per channel
    // Filter for 'join' actions only (ignore 'leave')
    const stmt = this.db.prepare(`
      WITH membership_events AS (
        ...
        FROM messages
        WHERE kind = 'state'
          AND data LIKE '%_channelMembership%'
          AND json_extract(data, '$._channelMembership.member') = ?
      )
