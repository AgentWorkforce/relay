...
export function createProgram(options: { name?: string } = {}): Command {
  ...
  program
    .command('mcp')
    .description('Start the Agent Relay MCP server over stdio')
    .action(async () => {
      const { optionsFromEnv, startAgentRelayMcpStdio } = await import('./relaycast-mcp.js');
      await startAgentRelayMcpStdio(optionsFromEnv());
    });
