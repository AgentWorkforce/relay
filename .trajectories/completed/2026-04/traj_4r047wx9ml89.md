...
> **Started:** April 7, 2026 at 09:28 AM
> **Completed:** April 10, 2026 at 08:56 AM
...
### Moved theme switching from SiteNav to SiteFooter
- **Chose:** Moved theme switching from SiteNav to SiteFooter
- **Reasoning:** Keeps primary navigation focused on site navigation/actions while preserving a single global theme control in a lower-priority UI area.

### Added Discord footer social link
- **Chose:** Added Discord footer social link
- **Reasoning:** The footer already groups external community links there, so Discord belongs with the existing social icons rather than in primary navigation.

### Server-rendered the GitHub stars badge with Next revalidation
- **Chose:** Server-rendered the GitHub stars badge with Next revalidation
- **Reasoning:** Eliminates the empty-then-flash client fetch by shipping a cached count in the initial HTML while keeping it fresh on a 1 hour cadence.

### Simplified /blog index to a title-and-author list
- **Chose:** Simplified /blog index to a title-and-author list
- **Reasoning:** The index should scan quickly; a plain list removes card noise and makes the page function more like an archive than a marketing surface.

### Expanded blog archive rows with author, date, and estimated read time while removing the Blog title block
- **Chose:** Expanded blog archive rows with author, date, and estimated read time while removing the Blog title block
- **Reasoning:** Keeps /blog as a compact archive view while still surfacing the metadata people need to scan posts quickly.

### Moved blog archive metadata below each title with dot separators
- **Chose:** Moved blog archive metadata below each title with dot separators
- **Reasoning:** Reads more naturally as an archive list than as a pseudo-table and matches the simpler scan pattern you asked for.

### Moved the blog RSS link into the header beside the Blog title and removed the stacked divider
- **Chose:** Moved the blog RSS link into the header beside the Blog title and removed the stacked divider
- **Reasoning:** The RSS action belongs in the page header, and using only the hero divider avoids the duplicate rule lines above the archive list.
...
### Removed web/public/docs to stop preview routing collisions with /docs
- **Chose:** Removed web/public/docs to stop preview routing collisions with /docs
- **Reasoning:** SST/OpenNext treats top-level public directories as static route prefixes, so keeping public/docs caused preview environments to route /docs* to S3 instead of the Next app.
...
- Moved theme switching from SiteNav to SiteFooter: Moved theme switching from SiteNav to SiteFooter
- Added Discord footer social link: Added Discord footer social link
- Server-rendered the GitHub stars badge with Next revalidation: Server-rendered the GitHub stars badge with Next revalidation
- Simplified /blog index to a title-and-author list: Simplified /blog index to a title-and-author list
- Expanded blog archive rows with author, date, and estimated read time while removing the Blog title block: Expanded blog archive rows with author, date, and estimated read time while removing the Blog title block
- Moved blog archive metadata below each title with dot separators: Moved blog archive metadata below each title with dot separators
- Moved the blog RSS link into the header beside the Blog title and removed the stacked divider: Moved the blog RSS link into the header beside the Blog title and removed the stacked divider
...
- Removed web/public/docs to stop preview routing collisions with /docs: Removed web/public/docs to stop preview routing collisions with /docs
