...
- `agent-relay node up|down|status|metrics|tail`, `node agent …`, and `node workflow run|logs|sync` unify `local up` and `fleet serve` under one command group. `node up [--config <file>]` starts the broker as the node's `broker` provider (advertising `spawn:<harness>`/`release` capacity) and serves the project's capability definitions as further providers connected directly to the engine: an `agent-relay.{ts,tsx,mts,cts,js,mjs,cjs}` file via `@agent-relay/fleet`, and an `agent-relay.py` as a supervised `python` child. With no definition file the broker's capacity alone brings the node online.
- **Node providers** — a node's capabilities are hosted by one or more providers, each connected directly to the engine with the node's token. `@agent-relay/fleet` `serveNode(options)` / `startServeNode(options)` (returning `RunningNode { stop(), done }`) register a `defineNode(...)` definition's capabilities against the engine via `@relaycast/sdk`'s node-provider client and execute their invokes; `ctx.spawnAgent` delegates to the node's capacity executor and `ctx.sendMessage` posts through the canonical message route.
- Actions are node-addressed: `POST /v1/nodes/:node/actions/:name/invoke`. A capability named `spawn:<harness>` registered by a provider shadows the broker's native spawn, so a handler in any language can transform the spawn spec before delegating to the broker.
- Python `agent_relay.node.NodeProvider.from_enrollment()` builds a node provider from the enrollment (`RELAY_NODE_TOKEN`/`RELAY_NODE_ID`/`RELAY_BASE_URL` env, `fleet-enrollments.json` fallback), wrapping relaycast-sdk's `NodeProvider`. Install with the `node` extra.
