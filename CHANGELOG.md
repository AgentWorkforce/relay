...
- `@agent-relay/sdk` harness definitions resolve named agent runtimes into broker-executable `pty` or `headless` plans, so custom CLIs like Qwen can be configured without Rust changes.
