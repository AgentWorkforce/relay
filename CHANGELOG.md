...
- `agent-relay integration subscribe` is now idempotent and supports multiple channels per provider. Inbound webhooks are named per channel (`relayfile:<provider>:<channel>`) instead of one-per-provider, so subscribing a second Slack channel no longer collides on the unique `(workspace, webhook name)` index. Re-running replaces an existing binding (and clears webhooks orphaned by an earlier partial run) instead of failing with a `UNIQUE constraint` error, and a failed rollback now warns instead of being silently swallowed. The relay channel id is also normalized (`#general` → `general`) consistently across the webhook, subscription filter, relayfile bind, and writeback-secret lookup.
