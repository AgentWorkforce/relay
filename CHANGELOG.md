...
- The broker retries a transient Relaycast 5xx during startup (workspace creation and agent registration) instead of exiting on the first one. relaycast 8.0.0 stopped retrying unkeyed POSTs, so a single upstream 503 was killing `node up` and every packaging smoke job.
