...
- `sdk-swift` `AgentClient` adds threaded conversations for TS participant-SDK parity: `thread(_:limit:)` fetches a parent message and its replies as `RelayThread`, and `reply(to:message:)` posts a threaded reply.
