...
- `agent-relay` MCP `add_agent` now fail-fast checks `spawn:<cli>` fleet placement and, when no live hosted node advertises the capability, falls back to the already-running local broker so agents spawned under `agent-relay up` can delegate without hitting `No live node provides spawn:<cli>`.
