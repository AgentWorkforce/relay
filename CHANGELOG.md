...
- Two-node fleet E2E (`tests/e2e/fleet`, `npm run test:e2e`, `Fleet E2E` CI workflow): boots a real relaycast engine plus two `agent-relay fleet serve` nodes and asserts the live control wire — boot/register (real broker `Authorization: Bearer` node auth), capability-filtered roster, cross-node action dispatch + ack, declarative trigger fire-once with loop guard, sidecar crash → `handlers_live` drop → restart, and capability-based spawn placement.
