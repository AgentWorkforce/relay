...
- `agent-relay node agent attach --node` no longer fails with "has no terminal transport" on long-lived nodes; the fleet terminal connection now detects a silently dropped network link and reconnects automatically.
