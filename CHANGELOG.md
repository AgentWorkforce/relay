...
- `agent-relay-broker` node id is now derived from the machine-id seed plus a hash of the broker's working directory, so multiple brokers / workspaces on one host no longer collide on `create_node` (stable across restarts in the same directory, distinct across directories).
