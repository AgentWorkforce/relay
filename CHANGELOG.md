...
- `agent-relay integration subscribe|unsubscribe` now persists and retires each relayfile-cloud inbound webhook subscription during replacement, rollback, and normal removal. Lifecycles are crash-safe, concurrency-safe, and retryable: a durable project-local journal takes a per-resource lease shared by subscribe and unsubscribe, records an attempt (with deterministic recovery keys for all three created resources and a pinned relayfile workspace) before any remote create, records every failed delete for retry on later runs with active-binding safeguards, and refuses to run a lifecycle whose predecessor it cannot yet reconcile. (Full crash recovery of interrupted cloud creates activates once the v3 @relayfile/client is published and pinned; until then those attempts are retained fail-closed.)
