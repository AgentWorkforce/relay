...
- `node agent attach --mode drive|passthrough` truncates its status line to the terminal width, preventing repeated repaints from scrolling away agent output.
- `node agent list` stops reporting an exited or never-ready harness as `working` and cleans up its orphaned wrapper process.
