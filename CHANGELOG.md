...
- `agent-relay-broker` recovers from an observer-token name conflict on `POST /api/observer-token`: when a token under the requested name already exists, it rotates that same-named token to return fresh usable credentials instead of failing, so repeat mints under a fixed name succeed. It only rotates a token whose scopes and filters match the endpoint's read-only contract, propagating the conflict otherwise; create and recovery share one timeout budget so the response stays within the request deadline.
