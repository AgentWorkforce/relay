...
- `agent-relay node up` no longer lets a second local broker silently steal a live node's Cloud delivery socket: a machine-global node claim (`~/.agentworkforce/relay/node-claims/`) is taken by exclusive create before any broker is spawned, so it refuses an enrolled node id another live broker holds — naming that broker's pid and state directory — instead of registering first and failing after. Concurrent starts cannot both win the claim, an orphaned broker whose supervisor was killed still counts as holding its node, and the claim is released only once every process it protects is observed to exit. It covers a node id the broker can authenticate from its cached token, not just one with `RELAY_NODE_TOKEN` set (`--force` overrides, stale claims from a crashed broker never block a restart). The claim is fenced by a descriptor the broker child inherits at spawn, so a supervisor killed before the broker has published anything still leaves the node id guarded, and a released claim retires its generation number rather than freeing it for reuse. A startup that fails after the broker child exists now stops that child and verifies its exit, so a failed start cannot leave an untracked broker running. A live broker is recognised by the identity of the executable it is running rather than by its filename, so a broker installed under an operator-chosen name (`AGENT_RELAY_BIN` / `BROKER_BINARY_PATH`) is no longer mistaken for an unrelated process and stripped of its node id. That recognition now survives a launcher script: a broker started through a shell wrapper runs as its interpreter and then as whatever binary the wrapper `exec`s, and such a live, fenced child is still identified — by the launcher the claim recorded appearing in its command line, and by the child pid the start records the moment it spawns — instead of reading as an unrelated process whose node id the next start may take.
...
- Plumb repos: readonly string[] through EnsureCloudFleetSandboxInput
...
- `agent-relay node agent attach --node` now tolerates 30-second control-plane responses and retries transient reachability failures inside a 90-second overall budget.
- `agent-relay node agent attach --node` errors now report the node reference, resolved node ID when available, redacted endpoint, timeout, and attempt count.
...
- Serialize terminal worker writes
...
- Spawned PTY agents keep their initial brief queued until the harness exposes a proven input prompt, preventing slow CLI startup from losing the brief and leaving a registered agent idle.
- Spawned Codex PTY agents accept the CLI's directory-trust interstitial before readiness, so a queued initial brief reaches the real input prompt instead of remaining parked behind the startup menu.
