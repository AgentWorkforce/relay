...
- `agent-relay-broker` fleet node heartbeats now include the node roster snapshot and heartbeat timestamp so `nodes.list()` can report live load, active agents, capabilities, and liveness after reconnects.
