...
- **`spawn-cloud-swarm` skill and install hook in `agent-relay cloud connect claude`**: Bundles a Claude skill (`skills/spawn-cloud-swarm/SKILL.md`) that drives `cloud.local-mount.ensure` → N×`cloud.agent.spawn` (with `QUOTA_EXCEEDED` backoff) → 5-second polling on `cloud.local-mount.status` + `cloud.agent.list` → teardown defaulting to persist. The `cloud connect` action now drops the skill at `~/.claude/skills/spawn-cloud-swarm/SKILL.md` (0644) when the provider normalizes to `anthropic`. A new MCP preflight (`src/cli/lib/mcp-preflight.ts`) fails closed if the local relaycast does not expose the `cloud.local-mount.*` tool surface.
