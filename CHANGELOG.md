...
- `agent-relay node up` no longer lets a second local broker silently steal a live node's Cloud delivery socket: it refuses an enrolled node id another live broker on this machine already holds, naming that broker's pid and state directory and offering `node down`, a distinct enrollment, or `--force`. Ownership is a machine-global claim (`~/.agentworkforce/relay/node-claims/`) taken by exclusive create before any broker is spawned, so concurrent starts cannot both win it and nothing registers before it is held; it is released only once every process it protects is observed to exit, and a released generation number is retired rather than reused. Any explicit `RELAY_NODE_ID` is guarded — including one the broker would mint its own token for from workspace credentials — while `--local-only` claims nothing and a stale claim from a crashed broker or a reboot never blocks a restart. The claim is fenced by a descriptor the broker child inherits at spawn, so a supervisor killed before the broker has published anything still leaves the node id guarded; a live broker is recognised by the identity of the executable it runs rather than by its filename, and a broker started through a launcher script — or one whose start was killed before it could record the child it spawned — is still treated as holding its node instead of as an unrelated process the next start may take the node from. A startup that fails after the broker child exists now stops that child and verifies its exit, so a failed start cannot leave an untracked broker running.
