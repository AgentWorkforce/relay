...
- `agent-relay-broker` republishes the fleet node load snapshot after a worker is released, exits, or restarts, so `nodes.list()` reports accurate live load and active-agent counts (and the SDK `RelayNode` read-model exposes `nodeId` plus liveness fields) instead of going stale until the next periodic heartbeat.
