...
- `@agent-relay/sdk` exposes `AgentRelay.spawnHeadless({ cli, ... })` so high-level facade callers can launch headless harness agents with the same lifecycle hooks and handles as `spawnPty()`.
...
- Launch personas through the owning CLI or package and pass the resulting command to `relay.spawnPty(...)` or the SDK's headless CLI APIs; for AgentWorkforce personas, use `npx agentworkforce persona run <id>` once available so persona side effects remain CLI-owned.
