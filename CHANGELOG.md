...
- `agent-relay-broker` persists its inbound dedup cache (relaycast spawn/release control events and delivery read-acks) alongside pending deliveries and reloads it on startup, dropping expired entries, so a crash + restart no longer re-processes those already-seen control events and read-acks.
