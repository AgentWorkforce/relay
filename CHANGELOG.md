...
- Broker `POST /api/observer-token` mints idempotently: if an active observer token with the requested name already exists, it is rotated (returning fresh token material) instead of blindly re-created — repeat mints (e.g. re-opening Pear's "Join as observer" link) no longer fail with `observer_token_name_conflict` (409), since token names are unique per workspace.
