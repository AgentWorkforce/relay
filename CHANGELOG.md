...
- `agent-relay integration subscribe` (and `--list`) no longer fail with `flag provided but not defined: -json`. `listBindings` was invoking `relayfile integration bind --list --json`, but that command defines no `--json` flag and already emits JSON; the flag is dropped. This unblocks `subscribe`, which reads bindings on every run.
