 ...
 *
 * Usage:
 *   node --experimental-strip-types flows/diagnose/orchestration.spec.ts --out <path>
...
function reviewTask(reviewer: 'claude' | 'codex', final: boolean): string {
  ...
  return [
    ...
    'Check that every failed static gate and every one of the 158 diagnosis coverage rows is represented by a bidirectional bug/unknown mapping, and that snapshot/prerelease qualification cannot pass on a stale image.',
