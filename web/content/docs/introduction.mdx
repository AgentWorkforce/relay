...
- **Create and release sessions when a harness supports it.** The managed boundary lives in the runtime package, while core keeps creation as an action such as `agent.create`.
...
- `@agent-relay/runtime` when Agent Relay needs to manage a session boundary for Claude Code, Codex, OpenCode, OpenClaw, or another harness.
