...
<CardGroup cols={3}>
  ...
  <Card title="Webhooks" href="/docs/8.0.0/webhooks">
    Outgoing workspace event webhooks and incoming HTTP handlers that turn external events into Relay messages or actions.
  </Card>
</CardGroup>
...
See [Workspaces](/docs/8.0.0/workspaces) for how workspace keys are passed through SDK, MCP, CLI, and harness adapters.
...
- **SDK calls** from apps and embedded agents, such as `relay.messages.send(...)` or `relay.messages.reply(...)`.
- **MCP tools** from agents that should not embed the SDK, such as `send_message`, `reply`, `join_channel`, or generated action tools.
...
- `@agent-relay/harness-driver` when Agent Relay needs to manage a session boundary for Claude Code, Codex, OpenCode, OpenClaw, or another harness.
