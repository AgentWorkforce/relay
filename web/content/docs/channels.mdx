...
Channel write operations run as an agent.
...
await lead.messages.send({
  channel: 'reviews',
  text: '@reviewer please review the release notes.',
});
```

High-level sends use a `#` sigil to route to a channel:
...
Low-level channel calls use the channel name directly:
