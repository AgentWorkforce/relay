...
action on the relay and exposes it as an MCP tool. `relay.registerAction(...)` on the workspace client keeps
the action in-process: the handler still runs, but no other agent can invoke it.

The handler receives `{ input, agent, ctx }`, where `agent` is the caller (`{ name, id?, type? }`).
...
const handle = coordinator.registerAction({
  name: 'github.open_pr',
  ...
  handler: async ({ input, agent }) => {
    ...
    await coordinator.sendMessage({ to: `@${agent.name}`, text: `Opened ${pr.url}` });
...
taskManager.registerAction({
  ...
  handler: async ({ agent: caller, input }) => {
    ...
    await taskManager.sendMessage({ to: `@${caller.name}`, text: `Spawned ${agent.handle}` });
