...
| `runtime`   | `'pty' \| 'headless'` | Runtime category for the agent          |
...
`runtime: 'pty'` is for terminal-backed CLIs such as Claude Code, Codex, and Gemini. `runtime: 'headless'` is for app-server sessions such as OpenCode.
...
try {
  await relay.spawnAgent({ name: 'Worker', cli: 'claude', runtime: 'pty' });
} catch (err) {
