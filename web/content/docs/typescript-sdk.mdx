...
```typescript file="structured-result.ts"
import { AgentRelay } from '@agent-relay/sdk';
import { z } from 'zod';
...
You can also pass `jsonSchema` directly when you only want to describe the expected payload to the spawned agent. Use `relay.onAgentResult` to observe all structured results globally.
...
relay.onAgentResult    = (result: AgentResult) => { ... }
```
