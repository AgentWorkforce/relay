...
| `client.registerHarness(name, config)` | `PUT /api/harnesses/{name}` | Register a named harness config. |
| `client.listHarnesses()` | `GET /api/harnesses` | List named harness configs. |
