...
agent-relay new Alice claude --attach --mode relay       # auto-inject + human types alongside
...
| -------- | -------- | ----------- |
| `<name>` | yes | Agent name. Matches the positional shape of `drive`, `view`, `relay`, and `rm`. |
...
A bare `-n NAME CLI` invocation is also recognised — it routes to the spawn-and-attach composition in relay mode with `--ephemeral` on:
...
```bash
agent-relay new Alice claude --attach --mode relay --ephemeral
```
...
| Shape | Routes to |
| ----- | --------- |
| `agent-relay -n NAME CLI [args...]` | `agent-relay new NAME CLI [args...] --attach --mode relay --ephemeral` |
...
The `new`, `relay`, and `rm` verbs above round out the attach-style client surface alongside `view` and `drive`. Together they're the first-class client interface on the broker's HTTP and WebSocket routes.
