...
Spawn a new agent under the broker. Headless by default — the agent keeps running as a child of the broker and you attach later with `view`, `drive`, or `relay`. Pass `--attach` to immediately open a session.
...
agent-relay new -n Alice claude --attach                    # default mode: drive
agent-relay new -n Alice claude --attach --mode relay       # auto-inject + human types alongside
agent-relay new -n Alice claude --attach --mode view        # read-only
agent-relay new -n Alice claude --attach --ephemeral        # release on client exit
```
...
With `--attach --ephemeral`, the client registers a teardown that calls `DELETE /api/spawned/{name}` on clean detach, SIGINT, SIGTERM, or abnormal WebSocket close. Without `--ephemeral`, the agent keeps running after detach (the post-#864 default). `--mode` and `--ephemeral` are ignored without `--attach` (and produce a stderr warning if you try).
...
| `--mode <mode>` | no | With `--attach`: session mode (`view`, `drive`, or `relay`). Defaults to `drive`. |
...
```bash
agent-relay new -n Alice claude --attach --mode relay --ephemeral
```
...
| Shape | Routes to |
| ----- | --------- |
| `agent-relay -n NAME CLI [args...]` | `agent-relay new -n NAME CLI [args...] --attach --mode relay --ephemeral` |
...
The `new` / `relay` / `rm` verbs above complete the broker-owned agent model from [issue #864](https://github.com/AgentWorkforce/relay/issues/864). Together with `view` and `drive` they're the first-class client surface on the broker's existing HTTP/WS routes.
