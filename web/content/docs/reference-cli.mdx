...
The client auto-discovers the running broker by reading `.agent-relay/connection.json` in the current directory, then opens a WebSocket to the broker and filters for `worker_stream` events matching the named agent. ANSI escape sequences are preserved byte-for-byte so colors and cursor moves render correctly in your terminal.
...
The stream starts from now — `view` does not replay scrollback or render the agent's current screen on attach. To capture the current visible screen separately, use `agent-relay-broker dump-pty <name>`.

`view` is part of the broker-owned agent model described in [issue #864](https://github.com/AgentWorkforce/relay/issues/864). Companion verbs (`drive`, `relay`, `new`, `run`) are shipped in follow-up changes.
