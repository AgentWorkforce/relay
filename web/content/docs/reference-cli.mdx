...
| Command | Description |
| --- | --- |
| `agent-relay local up [flags]` | Start the local broker. |
...
| Command | Description |
| --- | --- |
| `agent-relay fleet serve [file] [--enrollment-token <token>]` | Serve a fleet node from a TS/JS definition file, or register via a Cloud enrollment token. |
...
| `agent-relay fleet enable` | Enable fleet nodes for the workspace. |
| `agent-relay fleet disable` | Disable fleet nodes for the workspace. |
| `agent-relay fleet inherit` | Use the deployment default for workspace fleet nodes. |
| `agent-relay fleet status` | Show local fleet broker and sidecar status. |
