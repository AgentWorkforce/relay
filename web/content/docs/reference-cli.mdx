...
| `agent-relay agent message hold <name>` | Hold new relay messages for a local broker agent. |
| `agent-relay agent message flush <name>` | Flush queued relay messages into a held local broker agent. |
| `agent-relay agent message auto <name>` | Resume automatic relay message injection for a local broker agent. |
...
Local attach mode is one of `view`, `drive`, or `passthrough`. Use `agent message hold`, `agent message flush`, and `agent message auto` to control drive-mode delivery from another terminal.
