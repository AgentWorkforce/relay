...
{
  "pending": [
    { "from": "Bob", "body": "please review #837", "queued_at_ms": 1715812345678, "event_id": "evt_abc" },
    { "from": "Carol", "body": "ping", "queued_at_ms": 1715812360123 }
  ]
}
```

`event_id` is included when the inbound message carried one (it is
omitted otherwise). The queue is bounded at 256 entries per worker —
when full, the oldest entry is evicted with a broker-side warning.
