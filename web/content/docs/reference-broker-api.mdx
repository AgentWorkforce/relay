...
| `GET`  | `/api/harnesses`      | List named in-memory harness configs.                             |
| `PUT`  | `/api/harnesses/{id}` | Register or replace a named harness config.                       |
...
`name` and `cli` are required. `harnessId` references a config registered with
`/api/harnesses/{id}`. `harnessConfig` sends a one-off concrete config. Provide
at most one of them. Returns `{ "success": true, ... }` on success or
`{ "success": false, "error": "..." }` with a non-2xx status on failure.
...
Clients may send raw text websocket frames, binary frames containing UTF-8
input, or JSON frames when an explicit envelope is easier. Binary frames are
decoded as UTF-8 and then processed the same way as raw text frames:

```json
{ "type": "pty_input", "data": "hello\n" }
```

The broker forwards one frame at a time and waits for the PTY write to be
accepted before reading the next frame, preserving per-agent ordering and
applying websocket/TCP backpressure. Each accepted frame receives:

```json
{ "type": "pty_input_ack", "name": "Alice", "bytes_written": 6 }
```

If the agent is missing, exits, or cannot accept PTY input, the stream emits
...
- **`auto_inject`** (default) — inbound messages auto-inject into the
  ...
  traffic on their own, or for an `agent-relay passthrough` client session where
