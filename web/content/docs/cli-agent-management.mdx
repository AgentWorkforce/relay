...
`hold` switches the local agent to manual delivery, `flush` drains pending relay messages into the agent, and `auto` resumes automatic injection. Attach sessions use `Ctrl+C` to detach locally.
