...
Each declared integration becomes a typed client on `ctx` (`ctx.github`, `ctx.slack`, `ctx.notion`, `ctx.linear`, `ctx.jira`). Cron-only personas get all integration fields as `undefined`.
