...
Under the hood, the plugin exposes these Agent Relay MCP tools to your Claude session. The `submit_result` tool appears only when a spawned task is configured with an Agent Relay result callback.
