...
`name` and `cli` are required. `harnessConfig` sends a concrete broker-executable
config for a custom PTY or headless harness. Returns `{ "success": true, ... }`
on success or `{ "success": false, "error": "..." }` with a non-2xx status on
failure.
