...
```json
{
  "args": [
    "--mcp-config",
    "{\"mcpServers\":{\"agent-relay\":{\"command\":\"npx\",\"args\":[\"-y\",\"agent-relay\",\"mcp\"]}}}"
  ],
  "sideEffectFiles": [],
  "agentToken": null
}
...
`droid` is high-risk for relay worker delegation. Current evals show it can route spawning tasks to its native Task capability instead of Agent Relay unless explicit disambiguation is present, so avoid using droid for coordinator or supervisor roles that need to spawn relay workers.
