...
type FileEventKind = 'read' | 'write' | 'list' | 'watch';
...
const FILE_EVENT_KINDS: FileEventKind[] = ['read', 'write', 'list', 'watch'];

const FILE_COLORS: Record<FileEventKind, string> = {
  read: '74, 144, 194',
  write: '99, 209, 139',
  list: '193, 103, 75',
  watch: '254, 188, 46',
...
const IDLE_TEXTS: Record<ModelProvider, string[]> = {
  claude: ['Waiting for file op...', 'Ready'],
  gemini: ['Waiting for file op...', 'Ready'],
  codex: ['Waiting for file op...', 'Ready'],
  copilot: ['Waiting for file op...', 'Ready'],
  opencode: ['Waiting for file op...', 'Ready'],
};

const WORKING_TEXTS: Record<ModelProvider, string[]> = {
  claude: ['Reading /src/auth.ts...', 'Watching /src/*.ts...', 'Indexing...', 'Writing /dist/server.js...'],
  gemini: ['Reading /config.json...', 'Indexing...', 'Watching /src/*.ts...', 'Writing /dist/client.js...'],
  codex: ['Writing /dist/out.js...', 'Reading /src/router.ts...', 'Indexing...', 'Watching /src/*.ts...'],
  copilot: ['Reading /app/page.tsx...', 'Writing /dist/chunk.js...', 'Watching /src/*.ts...', 'Indexing...'],
  opencode: ['Reading /src/db.ts...', 'Writing /tmp/cache.json...', 'Indexing...', 'Watching /src/*.ts...'],
};

const RELAYING_TEXTS: Record<ModelProvider, string[]> = {
  claude: ['Sending file to...', 'Syncing /dist/...', 'Streaming bytes...'],
  gemini: ['Sending file to...', 'Syncing /dist/...', 'Streaming bytes...'],
  codex: ['Sending file to...', 'Syncing /dist/...', 'Streaming bytes...'],
  copilot: ['Sending file to...', 'Syncing /dist/...', 'Streaming bytes...'],
  opencode: ['Sending file to...', 'Syncing /dist/...', 'Streaming bytes...'],
};

const NODE_POOL: { name: string; provider: ModelProvider; model: string }[] = [
  { name: 'FileServer', provider: 'claude', model: 'Opus' },
  { name: 'Reader', provider: 'gemini', model: '2.5 Pro' },
  { name: 'Writer', provider: 'codex', model: 'Codex-1' },
  { name: 'Indexer', provider: 'claude', model: 'Sonnet' },
  { name: 'Watcher', provider: 'copilot', model: 'GPT-4.1' },
  { name: 'Syncer', provider: 'opencode', model: 'Gemini' },
  { name: 'Archiver', provider: 'gemini', model: '2.5 Flash' },
  { name: 'Validator', provider: 'claude', model: 'Haiku' },
];
...
function getFileColor(kind: FileEventKind, alpha: number) {
  return `rgba(${FILE_COLORS[kind]}, ${alpha})`;
}
...
function spawnLandingToast(landingToasts: LandingToast[], x: number, y: number, kind: FileEventKind) {
  landingToasts.push({
    x,
    y: y - 30,
    text: pick(LANDING_TEXTS[kind]),
    kind,
    age: 0,
    maxAge: 100,
    alpha: 0,
...
function drawMessageTrail(ctx: CanvasRenderingContext2D, message: Message) {
  ...
  for (let trailIndex = message.trail.length - 1; trailIndex >= 0; trailIndex--) {
    ...
    ctx.fillStyle = getFileColor(kind, trailAlpha);
    ctx.fill();
  }
}
...
function drawFilePulse(ctx: CanvasRenderingContext2D, x: number, y: number, kind: FileEventKind) {
  const glow = getFileColor(kind, 1);
  const grad = ctx.createRadialGradient(x, y, 0, x, y, 32);
  grad.addColorStop(0, getFileColor(kind, 0.35));
  grad.addColorStop(0.5, getFileColor(kind, 0.08));
  grad.addColorStop(1, getFileColor(kind, 0));
  ctx.beginPath();
  ...
  if (kind === 'read') {
    const docX = x - 12;
    const docY = y - 10;
    drawRoundedRect(ctx, docX, docY, 24, 20, 4);
    ctx.fillStyle = getFileColor(kind, 0.85);
    ctx.fill();
    ctx.fillStyle = 'rgba(234, 230, 221, 0.92)';
    ctx.fillRect(docX + 5, docY + 5, 14, 2);
    ctx.fillRect(docX + 5, docY + 9, 11, 2);
    ctx.fillRect(docX + 5, docY + 13, 9, 2);
    return;
  }
  ...
  if (kind === 'write') {
    const docX = x - 11;
    const docY = y - 10;
    drawRoundedRect(ctx, docX, docY, 20, 22, 4);
    ctx.fillStyle = getFileColor(kind, 0.85);
    ctx.fill();
    ctx.fillStyle = 'rgba(234, 230, 221, 0.92)';
    ctx.fillRect(docX + 4, docY + 5, 10, 2);
    ctx.fillRect(docX + 4, docY + 9, 8, 2);
    ctx.save();
    ctx.translate(x + 2, y + 5);
    ctx.rotate(-0.7);
    ctx.fillStyle = 'rgba(234, 230, 221, 0.96)';
  ...
  if (kind === 'list') {
    ...
    const folderY = y - 8;
    ctx.beginPath();
    ctx.moveTo(folderX, folderY + 4);
    ctx.lineTo(folderX + 7, folderY + 4);
    ctx.lineTo(folderX + 10, folderY);
    ctx.lineTo(folderX + 24, folderY);
    ctx.lineTo(folderX + 26, folderY + 18);
    ctx.lineTo(folderX, folderY + 18);
    ctx.closePath();
    ctx.fillStyle = getFileColor(kind, 0.85);
    ctx.fill();
    ctx.fillStyle = 'rgba(234, 230, 221, 0.9)';
    ctx.fillRect(folderX + 6, folderY + 7, 12, 2);
    ctx.fillRect(folderX + 6, folderY + 11, 10, 2);
    return;
  }

  ctx.beginPath();
  ctx.ellipse(x, y, 13, 8, 0, 0, Math.PI * 2);
  ctx.fillStyle = getFileColor(kind, 0.82);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(x, y, 13, 8, 0, 0, Math.PI * 2);
  ctx.strokeStyle = 'rgba(234, 230, 221, 0.7)';
  ctx.lineWidth = 1;
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(x, y, 4, 0, Math.PI * 2);
  ctx.fillStyle = 'rgba(15, 27, 41, 0.85)';
  ctx.fill();
  ctx.beginPath();
...
function handleMessageArrival(
  ...
  if (message.kind && centers[message.to]) {
    spawnLandingToast(landingToasts, centers[message.to].cx, centers[message.to].cy, message.kind);
  }
...
function drawMessages(
  ...
  for (let messageIndex = messages.length - 1; messageIndex >= 0; messageIndex--) {
    ...
    if (message.t <= 1) {
      if (message.kind) {
        drawFilePulse(ctx, px, py, message.kind);
      }
      continue;
    }

    handleMessageArrival(message, nodes, messages, landingToasts, centers);
    messages.splice(messageIndex, 1);
  }
}
...
function drawLandingToasts(ctx: CanvasRenderingContext2D, landingToasts: LandingToast[]) {
  for (let i = landingToasts.length - 1; i >= 0; i--) {
    ...
    const pillHeight = 24;
    const pillX = toast.x - pillWidth / 2;
    const pillY = toast.y - pillHeight / 2;
    const pillRadius = pillHeight / 2;

    drawRoundedRect(ctx, pillX, pillY, pillWidth, pillHeight, pillRadius);
    ctx.fillStyle = 'rgba(15, 27, 41, 0.9)';
    ctx.fill();
    ctx.strokeStyle = getFileColor(toast.kind, 0.35);
    ctx.lineWidth = 1;
    ctx.stroke();

    ctx.fillStyle = getFileColor(toast.kind, 0.95);
...
export function RelayfileAnimation() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const ns = nodesRef.current;
      const ms = messagesRef.current;
      const landingToasts = landingToastsRef.current;
      ...
      drawMessages(ctx, ns, ms, centers, landingToasts);
      drawGlowRings(ctx, ns, centers);
      drawLandingToasts(ctx, landingToasts);
