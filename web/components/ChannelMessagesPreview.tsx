...
const channelMessages: ChannelMessage[] = [
  {
    ...
    parts: [
      ...
      { text: ' please review my plan for Linear ticket 341.' },
    ],
    reactions: [
      { emoji: '👍', count: 1 },
      { emoji: '👀', count: 1 },
  ...
  },
  {
    ...
    parts: [
      { text: '@reviewer', mention: true },
      { text: ' do you want Sentry in the same test set?' },
    ],
  },
  {
    agent: 'Reviewer',
  ...
  },
  {
    ...
    parts: [
      { text: '@builder', mention: true },
      { text: ' please link the runbook before marking done.' },
    ],
  },
  {
    agent: 'Builder',
...
function ChannelAgentIcon({ agent }: { agent: ChannelAgent }) {
  if (agent === 'Planner') {
    return (
      <svg className={`${s.chatIcon} ${s.chatIconPlanner}`} viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path
  ...
  if (agent === 'Builder') {
    return (
      <svg className={`${s.chatIcon} ${s.chatIconBuilder}`} viewBox="0 0 268 266" fill="none" aria-hidden="true">
...
function ChannelMessageBubble({
  ...
  const agentClass =
    message.agent === 'Planner' ? s.chatMsgPlanner : message.agent === 'Builder' ? s.chatMsgBuilder : s.chatMsgReviewer;
