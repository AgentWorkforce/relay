...
When an agent starts, it connects to the broker and registers with its name. The broker maintains this connection until the agent disconnects or is explicitly released.
...
2. Wrapper detects pattern, extracts message
           |
           v
3. Wrapper sends SEND envelope to broker
           |
           v
4. Broker looks up "Bob" in registry
           |
           v
5. Broker sends DELIVER envelope to Bob's wrapper
           |
           v
...
The broker is the central message broker that routes all communication between agents.
...
The broker maintains persistent connections with each agent via Unix domain sockets. Key features:
...
The broker creates these files in `/tmp/agent-relay/`:
...
Bridging connects multiple project brokers together, enabling cross-project communication.
...
| ----------- | ---------------- | ------------------- |
| `HELLO`     | Client -> Broker | Initiate handshake  |
| `WELCOME`   | Broker -> Client | Confirm connection  |
| `SEND`      | Client -> Broker | Send a message      |
| `DELIVER`   | Broker -> Client | Deliver a message   |
