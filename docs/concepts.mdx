...
The daemon maintains persistent connections with each agent. Key features:
...
The daemon creates these files in the `.agent-relay/` project directory:
...
Agent Relay uses a file-based protocol for agent-to-daemon communication.
