...
All routes below live under `http://<host>:<port>`.

## Authentication

The broker requires an API key on every protected route. Pass it as
either header:
...
The expected token is read from the `RELAY_BROKER_API_KEY` environment
variable. If that variable is unset, the broker runs **unauthenticated**
— protected routes accept any request. In production, always set it.
