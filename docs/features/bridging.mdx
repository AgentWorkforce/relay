...
<AccordionGroup>
  <Accordion title="Bridge fails to connect">
    ...
    2. Check that `.agent-relay/` directories exist in each project
    3. Ensure both projects have been initialized with `agent-relay up`
  </Accordion>
...
The bridge client connects to each project's daemon and routes messages between them based on the project prefix in the address.
