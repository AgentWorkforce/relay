...
|  (React/Vue/etc) |           |  (RelayClient)   |              |  Broker          |
+------------------+           +------------------+              +------------------+
        ^                              ^
        |                              |
        v                              v
...
import { initializeRelay, disconnectRelay, destroyRelay, registerRelayIPC } from './relay-service';
...
app.whenReady().then(async () => {
  ...
  if (!connected) {
    console.warn('Relay broker not available - relay features disabled');
  }
});
...
export interface RelayAPI {
  ...
  getMetrics: (agentName?: string) => Promise<{ success: boolean; metrics?: unknown; error?: string }>;
  ...
  onChannelMessage: (
    callback: (data: { from: string; channel: string; body: string; envelope: unknown }) => void
  ) => () => void;

  onStateChange: (callback: (data: { state: string; connected: boolean }) => void) => () => void;
...
contextBridge.exposeInMainWorld('relay', {
  ...
  // Messaging
  sendMessage: (to, body, options) => ipcRenderer.invoke('relay:sendMessage', to, body, options),
  sendAndWait: (to, body, options) => ipcRenderer.invoke('relay:sendAndWait', to, body, options),
  broadcast: (body, options) => ipcRenderer.invoke('relay:broadcast', body, options),
  ...
  // Channels
  joinChannel: (channel, displayName) => ipcRenderer.invoke('relay:joinChannel', channel, displayName),
  leaveChannel: (channel, reason) => ipcRenderer.invoke('relay:leaveChannel', channel, reason),
...
export function useRelay() {
  ...
  const spawnAgent = useCallback(async (options: Parameters<typeof window.relay.spawn>[0]) => {
...
export function RelayPanel() {
  const { connected, state, messages, error, sendMessage, listAgents, spawnAgent, clearError } = useRelay();
...
Handle the case where the relay broker is not running:
...
export async function initializeRelayWithFallback(config: RelayConfig): Promise<boolean> {
  try {
    ...
    if (!connected) {
      console.warn('[Relay] Broker not available - running in offline mode');
      return false;
    }
    return true;
  } catch (error) {
...
```typescript
async function sendWithRetry(to: string, body: string, maxRetries = 3): Promise<boolean> {
...
For integration tests, start the relay broker and real Electron app:
...
6. **Error handling**: Gracefully handle broker unavailability
