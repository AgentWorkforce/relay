...
3. Agents respond through the relay broker
...
### 1. Start the Broker and Configure Zed

```bash
agent-relay up --zed
```

This starts the relay broker and auto-configures Zed's `~/.config/zed/settings.json` with the correct paths for your workspace.
...
### Bridge not connecting to broker

Ensure the relay broker is running:
...
const agent = new RelayACPAgent({
  ...
  socketPath: '/tmp/relay-broker.sock',
  debug: true,
  capabilities: {
  ...
  "agent_servers": {
    "Agent Relay": {
      ...
      "args": [
        ...
        "zed-bridge",
        "--socket",
        "/path/to/your/project/.agent-relay/relay.sock",
        "--debug"
      ],
      "env": {}
    }
