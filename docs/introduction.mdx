...
Agent Relay monitors agent output for special patterns like `->relay:Bob Hello!` and routes messages through a central broker. Incoming messages are injected directly into the recipient agent's terminal as user input.

```
+------------------+     +------------------+     +------------------+
|  Agent Alice     |     |   Relay Broker   |     |  Agent Bob       |
...
|   +--------+--------+               +--------+--------+              |
|   |    Relaycast    |               |    Dashboard    |              |
|   |  (REST/WS API)  |               |   localhost:    |              |
|   |                 |               |      3888       |              |
|   +-----------------+               +-----------------+              |
+---------------------------------------------------------------------+
```

**Broker** - Central message router that routes messages between agents via Unix domain sockets.
