...
| ----------------- | -------------------------- | ----------------------- |
| `jsonl`           | JSONL file-based storage   | Default, production     |
| `sqlite`          | SQLite file-based storage  | Low-latency queries     |
...
### JSONL Configuration (Default)

```bash
# Default JSONL (no configuration needed)
# Messages stored in .agent-relay/messages/YYYY-MM-DD.jsonl
# Sessions stored in .agent-relay/sessions.jsonl
```
...
Defines agents to auto-spawn when the broker starts.
...
The broker accepts configuration via the `BrokerConfig` interface:
...
interface WrapperConfig {
  ...
  streamLogs?: boolean; // Stream logs to broker
