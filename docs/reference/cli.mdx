...
Check the daemon status and list active agents.
...
Status: RUNNING
Active agents:
- Dashboard (cli: claude, cwd: /Users/you/myproject)
- Worker1 (cli: claude, cwd: /Users/you/myproject)
```
...
<Note>
For most use cases, we recommend using the **dashboard** at http://localhost:3888 or `agent-relay spawn`. Use `spawn` with advanced options for shadow agents or custom prefixes.
</Note>

```bash
agent-relay spawn [options] <command...>
```
...
agent-relay spawn claude

# With custom name
agent-relay spawn -n MyAgent claude

# With shadow agent for code review
agent-relay spawn -n Lead --shadow Reviewer --shadow-role reviewer claude

# With explicit triggers
agent-relay spawn --shadow Auditor --shadow-role "SESSION_END,CODE_WRITTEN" claude

# Debug mode
agent-relay spawn -d codex
```
...
| `agent-relay gc` | Clean up orphaned agent processes |
