...
The simplest way to get started - creates an in-process broker automatically:
...
### With External Broker (Node.js)

Connect to a running broker via Unix socket:
...
Spawn and manage agents via the broker:
...
Distributed decision-making across agents. **Note:** Consensus requires an external broker - it's not available in standalone mode.
...
Stream logs to the broker for dashboard display:
...
<Note>
  When using `transportInstance`, auto-reconnection is **not supported** because the SDK cannot recreate an
  externally-provided transport. Use `transport` options instead, or handle reconnection manually.
</Note>
...
For simple use cases, create an in-process broker:
...
The SDK includes utilities for discovering broker sockets and cloud workspaces.
...
```typescript
// Find the broker socket for current directory
...
try {
  ...
  if (err instanceof BrokerNotRunningError) {
    console.log('Start the broker with: agent-relay up');
...
```typescript
import { encodeFrame, encodeFrameLegacy, FrameParser, MAX_FRAME_BYTES } from '@agent-relay/sdk';
...
- [API Reference](/reference/api) - Full API including Broker class
