...
For multi-step orchestration, use the `workflow()` builder:
...
const result = await workflow('my-feature')
  ...
  .agent('coder', { cli: 'codex', model: Models.Codex.GPT_5_3_CODEX })
  .step('plan', { agent: 'planner', task: 'Create a plan' })
  .step('implement', { agent: 'coder', task: 'Build it', dependsOn: ['plan'] })
  .run();
```

See [TypeScript SDK](/workflows/typescript-sdk) and [Python SDK](/workflows/python-sdk) for full workflow docs.
...
- [TypeScript SDK](/workflows/typescript-sdk) — Workflow builder API
- [Python SDK](/workflows/python-sdk) — Python workflow examples
